...
func BenchmarkSQLMaterializedRuntimeJoinFilter(b *testing.B) {
	cases := []struct {
		name       string
		leftRows   int
		rightRows  int
	...
	for _, benchmark := range cases {
		benchmark := benchmark
		b.Run(benchmark.name+"/baseline", func(b *testing.B) {
			benchmarkSQLMaterializedRuntimeJoinFilter(b, benchmark, hatSql.QueryOptions{})
		})
		b.Run(benchmark.name+"/runtime_filter", func(b *testing.B) {
			benchmarkSQLMaterializedRuntimeJoinFilter(b, benchmark, hatSql.QueryOptions{RuntimeJoinBloomFilter: true})
		})
	}
}
...
func benchmarkSQLMaterializedRuntimeJoinFilter(b *testing.B, benchmark struct {
	name       string
	leftRows   int
	rightRows  int
	hotKey     bool
	...
	b.Helper()
	resolver := newNonStreamingRuntimeJoinFilterBenchmarkResolver(benchmark.leftRows, benchmark.rightRows, benchmark.hotKey)
	query := "FROM CACHE('left') AS l JOIN CACHE('right') AS r ON l.k = r.k SELECT l.id, r.id AS right_id"
	b.ReportAllocs()
	for b.Loop() {
		result, err := hatSql.ExecuteSQLQueryContext(context.Background(), query, resolver, options)
...
func newNonStreamingRuntimeJoinFilterBenchmarkResolver(leftRows, rightRows int, hotKey bool) *nonStreamingRuntimeJoinFilterResolver {
	left := make([]hatSql.SQLRow, 0, leftRows)
	for index := 0; index < leftRows; index++ {
		key := fmt.Sprintf("probe-%06d", index)
		if hotKey {
	...
	return &nonStreamingRuntimeJoinFilterResolver{sources: map[string][]hatSql.SQLRow{"left": left, "right": right}}
}
