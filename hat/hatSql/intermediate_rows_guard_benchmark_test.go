...
func BenchmarkSQLIntermediateRowsGuarded(b *testing.B) {
	...
	options := SQLQueryOptions{MaxIntermediateRows: 1024}
	b.ReportAllocs()
	for index := 0; index < b.N; index++ {
		result, err := ExecuteSQLQueryContext(context.Background(), query, resolver, options)
		if err != nil || len(result.Rows) != 512 {
			b.Fatalf("guarded query result=%d err=%v, want 512 rows", len(result.Rows), err)
		}
	}
}
