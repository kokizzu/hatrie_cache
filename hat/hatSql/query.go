...
type SQLQueryOptions struct {
	...
	SpillBloom bool
	// RuntimeJoinBloomFilter enables an opt-in direct equality-join fast path
	// with a bounded Bloom filter when it can skip probe work. It is disabled by
	// default; unsupported query shapes and index-backed joins retain their
	// established executor paths.
...
func sqlStreamScalarExpr(expr sqlExpr) bool {
	...
	case "literal", "field":
		return true
	case "func":
		if expr.name != "VALID_AT" || len(expr.args) != 3 {
			return false
		}
...
func sqlGlobalStreamAggregates(query *sqlQuery) ([]sqlStreamAggregate, bool) {
	...
	for index, item := range query.selects {
		...
		case "AUTO_COUNT_DISTINCT", "APPROX_COUNT_DISTINCT", "APPROX_COUNT_DISTINCT_STATE", "APPROX_COUNT_DISTINCT_MERGE", "APPROX_PERCENTILE", "APPROX_TDIGEST_PERCENTILE", "APPROX_TDIGEST_PERCENTILE_STATE", "APPROX_TDIGEST_PERCENTILE_MERGE":
		...
		case "COUNT_STATE", "SUM_STATE", "AVG_STATE", "MIN_STATE", "MAX_STATE", "COUNT_MERGE", "SUM_MERGE", "AVG_MERGE", "MIN_MERGE", "MAX_MERGE":
			...
			aggregate.arg = argument
		case "ARGMAX_STATE", "ARGMIN_STATE", "ARGMAX_MERGE", "COUNT_STATE_IF", "SUM_STATE_IF", "AVG_STATE_IF", "MIN_STATE_IF", "MAX_STATE_IF", "COUNT_MERGE_IF", "SUM_MERGE_IF", "AVG_MERGE_IF", "MIN_MERGE_IF", "MAX_MERGE_IF", "ARGMAX_STATE_IF", "ARGMIN_STATE_IF", "ARGMAX_MERGE_IF", "ARGMIN_MERGE_IF", "ARGMIN_MERGE":
...
func (aggregate *sqlStreamAggregate) addWithGroup(group []sqlExecRow, row sqlExecRow) error {
	if aggregate.filter != nil {
		value := evalSQLExpr(*aggregate.filter, []sqlExecRow{row}, row)
		if err := sqlExpressionError(value); err != nil {
			return err
		}
		if !sqlTruthy(value) {
...
func executeSQLSpillHashJoin(query *sqlQuery, resolver SQLSourceResolver, control *sqlExecutionControl, metrics *sqlExecutionMetrics) (SQLQueryResult, bool, error) {
	...
	rightParts, err := newSQLSpillHashPartitions(control.options.SpillDirectory, "hatrie-sql-hash-right-*", &available, paths, control)
	if err != nil {
	...
	for index := range leftParts {
		...
		processRightChunk := func(right []sqlSpillHashInput) error {
			...
			return visitSQLSpillHashPartition(leftParts[index], control, func(leftEntry sqlSpillHashInput) error {
	...
	rows, err := sqlMergeSpillRunsToRows(outputRuns, []sqlOrder{{}}, 0, -1, control, false)
	if err != nil {
		return SQLQueryResult{}, true, err
	}
	if metrics != nil {
...
func sqlRuntimeJoinFilterStreamable(query *sqlQuery, resolver SQLSourceResolver, control *sqlExecutionControl) (bool, error) {
	if query == nil || resolver == nil || control == nil || !control.options.RuntimeJoinBloomFilter || control.options.MaxJoinBytes > 0 || control.options.Workers > 0 || query.from == nil || query.sample != nil || len(query.ctes) != 0 || len(query.unions) != 0 || len(query.joins) != 1 || query.where.kind != "" || query.prewhere.kind != "" || query.having.kind != "" || query.distinct || len(query.groupBy) != 0 || len(query.orderBy) != 0 || query.offset != 0 || query.limit >= 0 || sqlQueryHasAggregate(query) || sqlQueryHasWindow(query) || sqlQueryHasSubqueryExpression(query) || query.indexHint.Mode != "" {
		return false, nil
	}
	join := query.joins[0]
...
// sqlRuntimeJoinFilterMaterialized limits the materialized runtime-filter path
// to the same direct inner equality shape as the streaming path. It is kept
// separate so a resolver that can stream continues to use the lower-allocation
// implementation above, while legacy materialized resolvers can still avoid
// wrapping every unmatched probe row in the general executor.
func sqlRuntimeJoinFilterMaterialized(query *sqlQuery, resolver SQLSourceResolver, control *sqlExecutionControl) (bool, error) {
	if query == nil || resolver == nil || control == nil || !control.options.RuntimeJoinBloomFilter || control.options.MaxJoinBytes > 0 || control.options.Workers > 0 || query.from == nil || query.sample != nil || len(query.ctes) != 0 || len(query.unions) != 0 || len(query.joins) != 1 || query.where.kind != "" || query.prewhere.kind != "" || query.having.kind != "" || query.distinct || len(query.groupBy) != 0 || len(query.orderBy) != 0 || query.offset != 0 || query.limit >= 0 || sqlQueryHasAggregate(query) || sqlQueryHasWindow(query) || sqlQueryHasSubqueryExpression(query) || query.indexHint.Mode != "" {
		return false, nil
	}
	if _, streaming := resolver.(SQLStreamSourceResolver); streaming {
		return false, nil
	}
	join := query.joins[0]
	if join.kind != "INNER" || join.source.lateral || query.from.kind != "CACHE" || join.source.kind != "CACHE" || len(query.from.fieldTypes) != 0 || len(join.source.fieldTypes) != 0 || len(query.selects) == 0 {
...
// executeSQLRuntimeJoinFilterMaterialized applies the runtime Bloom filter to
// raw rows returned by a legacy materialized resolver. Exact hash buckets still
// decide matches, so Bloom false positives cannot change SQL results.
func executeSQLRuntimeJoinFilterMaterialized(query *sqlQuery, resolver SQLSourceResolver, control *sqlExecutionControl, metrics *sqlExecutionMetrics) (SQLQueryResult, bool, error) {
	eligible, err := sqlRuntimeJoinFilterMaterialized(query, resolver, control)
	if err != nil {
		return SQLQueryResult{}, true, err
	}
	if !eligible {
	...
	rightSourceRows, err := resolveSQLSourceContext(control.ctx, resolver, join.source.kind, join.source.key)
	if err != nil {
		return SQLQueryResult{}, true, err
	}
	if len(rightSourceRows) > maxRows {
		return SQLQueryResult{}, true, fmt.Errorf("SQL source %q exceeds the %d row limit", join.source.alias, maxRows)
	}
	rightBuckets := make(map[string]sqlExecRow, len(rightSourceRows))
	rightDuplicates := make(map[string][]sqlExecRow)
	for _, row := range rightSourceRows {
		if err := control.addJoinWork(1); err != nil {
			return SQLQueryResult{}, true, err
		}
		key, ok := sqlHashJoinKey(row[rightField])
		...
		right := newSQLSingleSourceExecRow(join.source.alias, row)
		if _, exists := rightBuckets[key]; exists {
			rightDuplicates[key] = append(rightDuplicates[key], right)
		} else {
			rightBuckets[key] = right
	...
	leftSourceRows, err := resolveSQLSourceContext(control.ctx, resolver, query.from.kind, query.from.key)
	if err != nil {
		return SQLQueryResult{}, true, err
	}
	if len(leftSourceRows) > maxRows {
		return SQLQueryResult{}, true, fmt.Errorf("SQL source %q exceeds the %d row limit", query.from.alias, maxRows)
	}
	columns := sqlColumns(query.selects)
	result := SQLQueryResult{Columns: columns}
	evaluationGroup := make([]sqlExecRow, 1)
	leftRows, filterProbes, filterSkipped := 0, 0, 0
	for _, row := range leftSourceRows {
		if err := control.check(); err != nil {
			return SQLQueryResult{}, true, err
		}
		leftRows++
		...
		left := newSQLSingleSourceExecRow(query.from.alias, row)
		duplicates := rightDuplicates[key]
		for candidateIndex := -1; candidateIndex < len(duplicates); candidateIndex++ {
			if candidateIndex >= 0 {
				candidate = duplicates[candidateIndex]
			...
			result.Rows = append(result.Rows, projected)
			if len(result.Rows) > maxRows {
				return SQLQueryResult{}, true, fmt.Errorf("SQL join exceeds the %d row limit; add a more selective WHERE or ON condition", maxRows)
			}
		}
	}
	...
	if filterEnabled {
		filterDetail := fmt.Sprintf("Bloom filter right_rows=%d right_keys=%d probes=%d skipped=%d false_positive_rate=%.2f%%", len(rightSourceRows), len(rightBuckets), filterProbes, filterSkipped, sqlRuntimeJoinFilterFalsePositiveRate*100)
		metrics.record("RUNTIME JOIN FILTER", filterDetail, leftRows, filterProbes-filterSkipped, joinStarted)
	}
	joinDetail := "INNER JOIN " + sqlExplainSource(join.source) + " ON " + sqlExplainExpression(join.on)
	metrics.record("HASH JOIN", joinDetail, leftRows+len(rightSourceRows), len(result.Rows), joinStarted)
	return result, true, nil
}
...
func readSQLSpillHashPartition(partition sqlSpillHashPartition, control *sqlExecutionControl) ([]sqlSpillHashInput, error) {
...
func visitSQLSpillHashPartition(partition sqlSpillHashPartition, control *sqlExecutionControl, visit func(sqlSpillHashInput) error) error {
	file, err := os.Open(partition.path)
	if err != nil {
...
func sqlQueryOutputsTie(order []sqlOrder, left, right sqlQueryOutput) bool {
	for _, item := range order {
		leftValue := evalOutputOrder(item.expr, left.row, left.group)
		rightValue := evalOutputOrder(item.expr, right.row, right.group)
		if sqlCompare(leftValue, rightValue) != 0 {
			return false
		}
	}
	return true
...
func (p *sqlQueryParser) parseQueryInternal(stopRight bool) (*sqlQuery, error) {
	...
	if q.limitWithTies {
		if q.limit < 0 {
			return nil, p.diagnostic(p.current(), "LIMIT WITH TIES requires a finite non-negative LIMIT")
		}
		if len(q.orderBy) == 0 {
			return nil, p.diagnostic(p.current(), "LIMIT WITH TIES requires ORDER BY")
		}
	}
	if q.selects == nil {
...
func (p *sqlQueryParser) parsePrimary() (sqlExpr, error) {
	...
	if token.kind == sqlTokenIdentifier {
		...
		if p.current().kind == sqlTokenLeftParen {
			...
			if normalized, combinator, err := normalizeSQLAggregateIf(expr); combinator {
				if err != nil {
					return sqlExpr{}, p.diagnostic(token, err.Error())
				}
				if p.keyword("OVER") {
					return sqlExpr{}, p.diagnostic(token, "aggregate If window functions are not supported")
				}
				expr = normalized
			}
			if p.keyword("OVER") {
...
func executeSQLColumnarScan(q *sqlQuery, resolver SQLSourceResolver, control *sqlExecutionControl, metrics *sqlExecutionMetrics, outer *sqlExecRow) (SQLQueryResult, bool, error) {
	...
	if result, handled, err := executeSQLColumnarTopN(q, columnar, control, metrics, outer); handled {
		return result, true, err
	}
...
func executeSQLColumnarDictionaryDistinct(q *sqlQuery, columnar SQLColumnarSourceResolver, control *sqlExecutionControl, metrics *sqlExecutionMetrics, outer *sqlExecRow) (SQLQueryResult, bool, error) {
	...
	for _, code := range codes {
		...
		lastValue, haveLastValue = value, true
		if position < q.offset {
			position++
			continue
		}
		...
		row := SQLRow{result.Columns[0]: value}
		if control != nil && control.options.MaxResultBytes > 0 {
			resultBytes += sqlRowBytes(row)
			if resultBytes > control.options.MaxResultBytes {
				return SQLQueryResult{}, true, fmt.Errorf("SQL result byte budget exceeded: maximum %d bytes", control.options.MaxResultBytes)
...
func executeSQLColumnarTopN(q *sqlQuery, columnar SQLColumnarSourceResolver, control *sqlExecutionControl, metrics *sqlExecutionMetrics, outer *sqlExecRow) (SQLQueryResult, bool, error) {
	...
	for _, candidate := range candidates.items[start:end] {
		row := make(SQLRow, len(projectionFields))
		for selectIndex, item := range q.selects {
			row[result.Columns[selectIndex]], _ = batch.Value(item.expr.name, candidate.ordinal)
		}
		result.Rows = append(result.Rows, row)
	}
	if metrics != nil {
...
func sqlColumnarTopNFilterPlan(expr sqlExpr, alias string) (dictionaryField string, predicates []sqlColumnarNumericFilter, ok bool) {
	...
	collect = func(current sqlExpr) bool {
		...
		field, operator, value, numeric := sqlColumnarNumericPredicate(current, alias)
		if !numeric {
			return false
		}
...
func executeSQLColumnarDictionaryGroupAggregate(q *sqlQuery, columnar SQLColumnarSourceResolver, control *sqlExecutionControl, metrics *sqlExecutionMetrics, outer *sqlExecRow) (SQLQueryResult, bool, error) {
	...
	if metrics != nil {
		aggregateName := "COLUMNAR DICTIONARY GROUP AGGREGATE"
		if orderProjection >= 0 {
			aggregateName = "COLUMNAR DICTIONARY GROUP ORDER"
		}
...
func sqlColumnarDictionaryGroupAggregatePlan(q *sqlQuery, outer *sqlExecRow) (groupField string, projections []sqlColumnarDictionaryGroupProjection, fields []string, predicates []sqlColumnarNumericFilter, descending bool, ok bool) {
	...
	for index, projection := range ordered {
		candidate := sqlColumnarDictionaryGroupProjection{column: projection.column, group: projection.group, order: index == orderProjection}
		if projection.aggregate != nil {
...
func executeSQLQueryWithMetricsOuter(q *sqlQuery, resolver SQLSourceResolver, ctes map[string][]SQLRow, metrics *sqlExecutionMetrics, control *sqlExecutionControl, outer *sqlExecRow) (SQLQueryResult, error) {
	...
	if !finalSource && !sqlQueryHasWithFill(q) && q.limitBy == nil {
		...
		if result, handled, runtimeErr := executeSQLRuntimeJoinFilterMaterialized(q, resolver, control, metrics); handled {
			return result, runtimeErr
		}
	...
	if len(q.orderBy) > 0 && !indexOrdered && !limitByApplied {
		...
		if control != nil && (control.options.MaxSortBytes > 0 || control.operatorMemory != nil) && !sqlQueryHasWithFill(q) {
			...
			if control.options.MaxSortBytes > 0 && sortBytes > control.options.MaxSortBytes {
				...
				started = time.Now()
				rows, spillBytes, runs, err := sqlExternalSortRowsWithTies(spillRecords, q.orderBy, control.options.SpillDirectory, control.options.MaxSortBytes, control.options.MaxSpillBytes, q.offset, q.limit, q.limitWithTies, control, q.limitBy)
				if err != nil {
					return SQLQueryResult{}, err
				}
				result.Rows = rows
	...
	if !externallySorted {
		...
		if q.limit >= 0 && start+q.limit < end {
			...
			if q.limitWithTies && end > start {
				boundary := out[end-1]
				for end < len(out) && sqlQueryOutputsTie(q.orderBy, boundary, out[end]) {
					end++
				}
			}
		}
...
func sqlAppendExplainSteps(steps *[]SQLExplainStep, query *sqlQuery, prefix string, resolver SQLSourceResolver) {
	...
	*steps = append(*steps, SQLExplainStep{Node: prefix + "PROJECT", Detail: sqlExplainSelects(query.selects), Lineage: sqlExplainLineage(query.selects)})
	...
	if query.limit >= 0 || query.offset > 0 {
		detail := fmt.Sprintf("limit=%d offset=%d", query.limit, query.offset)
		if query.limitWithTies {
			detail += " with ties"
		}
		*steps = append(*steps, SQLExplainStep{Node: prefix + "LIMIT", Detail: detail})
	}
	for _, union := range query.unions {
		kind := union.kind
		if union.all {
...
func sqlSpillCipher(control *sqlExecutionControl) *hatCodec.StreamCipher {
	if control == nil {
		return nil
	}
...
func openSQLSpillReader(run sqlSpillRun, control *sqlExecutionControl) (*sqlSpillReader, error) {
	...
	reader := &sqlSpillReader{file: file, decoder: decoder}
...
func sqlMergeSpillRunsToRows(runs []sqlSpillRun, order []sqlOrder, offset, limit int, control *sqlExecutionControl, withTies bool, limitBy ...*sqlLimitBy) ([]SQLRow, error) {
	rows := []SQLRow{}
	position := 0
	boundaryKeys := []interface{}(nil)
	done := false
	...
	err := sqlMergeSpillRunsToVisit(runs, order, control, func(record sqlSpillOutput) error {
		...
		if limit < 0 || len(rows) < limit {
			...
			position++
			if withTies && limit >= 0 && len(rows) == limit {
				boundaryKeys = append(boundaryKeys[:0], record.Keys...)
			}
			return nil
		}
		if withTies && len(rows) > 0 && sqlSpillOutputKeysTie(record.Keys, boundaryKeys) {
			rows = append(rows, record.Row)
			position++
			return nil
		}
...
func sqlSpillOutputKeysTie(left, right []interface{}) bool {
	if len(left) != len(right) {
		return false
	}
	for index := range left {
		if sqlCompare(left[index], right[index]) != 0 {
			return false
		}
	}
	return true
...
func sqlExternalSortRows(records []sqlSpillOutput, order []sqlOrder, directory string, maxRunBytes, maxSpillBytes, offset, limit int, control *sqlExecutionControl, limitBy ...*sqlLimitBy) ([]SQLRow, int64, int, error) {
	return sqlExternalSortRowsWithTies(records, order, directory, maxRunBytes, maxSpillBytes, offset, limit, false, control, limitBy...)
}

func sqlExternalSortRowsWithTies(records []sqlSpillOutput, order []sqlOrder, directory string, maxRunBytes, maxSpillBytes, offset, limit int, withTies bool, control *sqlExecutionControl, limitBy ...*sqlLimitBy) ([]SQLRow, int64, int, error) {
	...
	rows, err := sqlMergeSpillRunsToRows(runs, order, offset, limit, control, withTies, limitBy...)
	if err != nil {
		return nil, 0, 0, err
	}
...
func openSQLSpillSetReader(run sqlSpillRun) (*sqlSpillSetReader, error) {
	...
	reader := &sqlSpillSetReader{file: file, decoder: decoder}
	if err := reader.next(); err != nil {
		_ = file.Close()
		return nil, err
	}
...
func openSQLSpillGroupReader(run sqlSpillGroupRun) (*sqlSpillGroupReader, error) {
	...
	reader := &sqlSpillGroupReader{file: file, decoder: decoder}
	if err := reader.next(); err != nil {
		_ = file.Close()
		return nil, err
	}
...
func sqlExprHasAggregate(expr sqlExpr) bool {
	...
	if expr.kind == "func" {
		switch expr.name {
		case "COUNT", "SUM", "AVG", "MIN", "MAX", "COUNT_OR_NULL", "SUM_OR_NULL", "AVG_OR_NULL", "MIN_OR_NULL", "MAX_OR_NULL", "ARGMAX", "ARGMIN", "AUTO_COUNT_DISTINCT", "APPROX_COUNT_DISTINCT", "APPROX_COUNT_DISTINCT_STATE", "APPROX_COUNT_DISTINCT_MERGE", "APPROX_PERCENTILE", "APPROX_TDIGEST_PERCENTILE", "APPROX_TDIGEST_PERCENTILE_STATE", "APPROX_TDIGEST_PERCENTILE_MERGE", "APPROX_TOP_K", "ARRAY_AGG", "GROUP_ARRAY", "GROUP_UNIQ_ARRAY", "MAP_AGG", "BITMAP_AGG", "COUNTIF", "COUNT_IF", "SUMIF", "SUM_IF", "AVGIF", "AVG_IF", "MINIF", "MIN_IF", "MAXIF", "MAX_IF", "ARGMAXIF", "ARGMAX_IF", "ARGMINIF", "ARGMIN_IF", "COUNT_STATE", "SUM_STATE", "AVG_STATE", "MIN_STATE", "MAX_STATE", "COUNT_MERGE", "SUM_MERGE", "AVG_MERGE", "MIN_MERGE", "MAX_MERGE", "ARGMAX_STATE", "ARGMIN_STATE", "ARGMAX_MERGE", "COUNT_STATE_IF", "SUM_STATE_IF", "AVG_STATE_IF", "MIN_STATE_IF", "MAX_STATE_IF", "COUNT_MERGE_IF", "SUM_MERGE_IF", "AVG_MERGE_IF", "MIN_MERGE_IF", "MAX_MERGE_IF", "ARGMAX_STATE_IF", "ARGMIN_STATE_IF", "ARGMAX_MERGE_IF", "ARGMIN_MERGE_IF", "ARGMIN_MERGE":
			return true
		}
		for _, arg := range expr.args {
...
func evalSQLExpr(expr sqlExpr, group []sqlExecRow, row sqlExecRow) interface{} {
	...
	case "func":
		...
		case "GROUPING":
			...
			return int64(0)
		case "VALID_AT", "PARSE_TIMESTAMP", "TIMESTAMP_ADD", "TIMESTAMP_DIFF":
		...
		case "AUTO_COUNT_DISTINCT", "APPROX_COUNT_DISTINCT", "APPROX_COUNT_DISTINCT_STATE", "APPROX_COUNT_DISTINCT_MERGE", "APPROX_PERCENTILE", "APPROX_TDIGEST_PERCENTILE", "APPROX_TDIGEST_PERCENTILE_STATE", "APPROX_TDIGEST_PERCENTILE_MERGE", "APPROX_TOP_K":
		...
		case "ARRAY_CONTAINS":
			...
			return false
		case "BITMAP_COUNT", "BITMAP_CONTAINS", "BITMAP_OR", "BITMAP_AND", "BITMAP_XOR":
			value, err := evalSQLBitmapFunction(expr, group, row)
			if err != nil {
				return sqlEvaluationFailure(err)
			}
			return value
		case "ARRAY_AGG", "GROUP_ARRAY":
			...
			return values
		case "BITMAP_AGG":
			bitmap, err := sqlAggregateBitmapValues(expr, group)
			if err != nil {
				return sqlEvaluationFailure(err)
			}
			return bitmap
...
func evalSQLExprBatch(expr sqlExpr, rows []sqlExecRow, functions SQLFunctionResolver) ([]interface{}, error) {
	...
	if expr.kind == "func" && expr.name == "VALID_AT" {
		return evalSQLValidAtBatch(expr, rows, functions)
	}
	if expr.kind == "func" && !sqlBuiltinFunction(expr.name) {
		if functions == nil {
