...
func sqlColumnarQueryRowsMatcher(query *sqlQuery, batch ColumnarBatch, functions SQLFunctionResolver) func(int) (bool, error) {
	...
	if kernel, nullPredicate := sqlColumnarNullPredicateKernelForBatch(query.where, query.from.alias, batch); nullPredicate {
...
func executeSQLColumnarScan(q *sqlQuery, resolver SQLSourceResolver, control *sqlExecutionControl, metrics *sqlExecutionMetrics, outer *sqlExecRow) (SQLQueryResult, bool, error) {
	...
	if q.where.kind == "" {
		...
		return result, true, nil
	} else if kernel, nullPredicate := sqlColumnarNullPredicateKernelForBatch(q.where, q.from.alias, batch); nullPredicate {
...
func executeSQLQueryWithMetricsOuter(q *sqlQuery, resolver SQLSourceResolver, ctes map[string][]SQLRow, metrics *sqlExecutionMetrics, control *sqlExecutionControl, outer *sqlExecRow) (SQLQueryResult, error) {
	...
	if outer == nil && len(q.unions) != 0 && !finalSource && control != nil && control.options.SubqueryResultCache == nil {
		baseQuery := *q
		baseQuery.unions = nil
		if sqlAutoNativeUnionBranchEligible(&baseQuery, resolver, control.options) {
			baseResult, handled, nativeErr := executeSQLAutoNativeDataflow(control.ctx, &baseQuery, resolver, control.options, control, metrics != nil)
			if handled {
				if metrics != nil {
					metrics.steps = append(metrics.steps, baseResult.Plan...)
				}
				if nativeErr != nil {
					return SQLQueryResult{}, nativeErr
				}
				return executeSQLUnionResult(q, baseResult, resolver, ctes, metrics, control)
			}
		}
	}
