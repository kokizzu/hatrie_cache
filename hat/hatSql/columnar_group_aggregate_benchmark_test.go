...
func BenchmarkSQLColumnarDictionaryGroupAggregate(b *testing.B) {
	...
	for iteration := 0; iteration < b.N; iteration++ {
		result, err := ExecuteSQLQueryParameters(ctx, "SELECT team, COUNT(*) AS total, SUM(score) AS sum FROM CACHE('items') WHERE team = 'team-c' AND score >= 50 GROUP BY team ORDER BY team", resolver, nil, SQLQueryOptions{})
