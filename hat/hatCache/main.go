...
type reusableIndexes = hatStorage.ReusableIndexes

func trimReusableTail[T any](values []T, reusables *reusableIndexes) []T {
	return hatStorage.TrimReusableTail(values, reusables)
}
...
type HatTrie struct {
	...
	sqlJSONPartialIndexes              map[string]map[string]*sqlJSONPartialIndex
	sqlPlannerStatisticsMu             sync.RWMutex
	sqlPlannerStatistics               map[string]sqlPlannerStatisticsEntry
	...
	sqlJSONIndexRebuildCheckpointStore SQLJSONIndexRebuildCheckpointStore
	sqlJSONIndexRebuildDurable         map[string]map[string]bool
	sqlJSONIndexRebuildInFlight        map[string]map[string]int
...
func (ht *HatTrie) recordWriteMutationLocked(keys ...string) {
	...
	for _, key := range keys {
		if generation, indexed := ht.sqlJSONIndexSourceGenerations[key]; indexed {
			ht.sqlJSONIndexSourceGenerations[key] = generation + 1
		}
	}
