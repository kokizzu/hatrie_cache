...
func BenchmarkSQLHatTrieColumnarStringBloomSegmentIN(b *testing.B) {
	trie := CreateHatTrie()
	b.Cleanup(trie.Destroy)
	var data strings.Builder
	data.Grow(620000)
	...
	data.WriteByte(']')
	trie.UpsertString("events", data.String())
	query := "FROM CACHE('events') AS event WHERE event.tag IN ('tag-19999', 'tag-999999') SELECT event.id"
	for warmup := 0; warmup < 2; warmup++ {
		if _, err := ExecuteSQLQuery(query, trie); err != nil {
			b.Fatalf("warm-up ExecuteSQLQuery() error = %v", err)
		}
