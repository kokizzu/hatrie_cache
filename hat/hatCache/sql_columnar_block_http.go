...
func monitoringSQLRequestAcceptsColumnar(accept string) bool {
	for _, item := range strings.Split(accept, ",") {
		mediaType, parameters, err := mime.ParseMediaType(strings.TrimSpace(item))
		if err != nil || !strings.EqualFold(mediaType, hatSql.SQLColumnarBlockStreamContentType) {
			continue
		}
...
func (handler *MonitoringHandler) handleSQLColumnarBlockStream(w http.ResponseWriter, r *http.Request, request SQLQueryRequest, query *hatSql.ParsedQuery, sources []string) {
	w.Header().Set("Content-Type", hatSql.SQLColumnarBlockStreamContentType)
	w.Header().Set("Vary", "Accept")
	writer := hatSql.NewSQLColumnarBlockStreamWriter(w, SQLQueryColumns(query), 0)
	...
	err := ExecuteSQLQueryRows(r.Context(), request.Query, resolver, request.Parameters, handler.options.SQLQueryOptions, func(_ []string, row SQLRow) error {
		...
		rows++
		if flusher != nil && writer.Progress() != before {
			flusher.Flush()
		}
		return nil
	})
