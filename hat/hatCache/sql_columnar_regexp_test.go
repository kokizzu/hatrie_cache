...
func TestSQLColumnarScanUsesRegexpFilter(t *testing.T) {
	trie := newTestTrie(t)
  ...
  {"id":3,"state":"queued-later"}
	...
	if !sqlColumnarRegexpPlanHasNode(explained.Plan, "COLUMNAR REGEXP FILTER") {
		t.Fatalf("plan = %#v, want COLUMNAR REGEXP FILTER", explained.Plan)
	}
}

func sqlColumnarRegexpPlanHasNode(plan []SQLExplainStep, node string) bool {
