...
func TestHatTrieSQLColumnarStringBloomSegmentSkipMatchesINQuery(t *testing.T) {
	t.Parallel()
	trie := newTestTrie(t)
	var data strings.Builder
	data.WriteByte('[')
	...
	query := "FROM CACHE('events') AS event WHERE event.tag IN ('tag-511', 'tag-511', 'tag-999999') SELECT event.id"
	for warmup := 0; warmup < 2; warmup++ {
		if _, err := ExecuteSQLQuery(query, trie); err != nil {
			t.Fatalf("warm-up ExecuteSQLQuery() error = %v", err)
		}
