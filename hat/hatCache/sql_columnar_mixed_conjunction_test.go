...
func TestSQLColumnarScanUsesDictionaryNumericConjunction(t *testing.T) {
	trie := newTestTrie(t)
	trie.UpsertString("jobs", `[
  {"id":1,"state":"queued"},
  {"id":2,"state":"running"},
  ...
  {"id":4,"state":"queued"}
	...
	query := "FROM CACHE('jobs') AS job WHERE job.state = 'queued' AND job.id >= 3 SELECT job.id"
	columnar, err := ExecuteSQLQuery(query, trie)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !sqlColumnarMixedPlanHasNode(explained.Plan, "COLUMNAR DICTIONARY NUMERIC FILTER") {
		t.Fatalf("plan = %#v, want COLUMNAR DICTIONARY NUMERIC FILTER", explained.Plan)
	}
}
