...
func (conn *SQLConn) Query(ctx context.Context, query string) (SQLQueryResult, error) {
	...
	body, err := json.Marshal(SQLQueryRequest{Query: query})
	...
	if strings.TrimSpace(conn.Token) != "" { req.Header.Set("Authorization", "Bearer "+conn.Token) }
	client := conn.Client
	...
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		...
		if readErr != nil { return SQLQueryResult{}, readErr }
	...
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil { return SQLQueryResult{}, err }
	return result, nil
}
...
func QueryRows[T any](ctx context.Context, conn *SQLConn, query string, visit func(T) error) (int, error) {
	if visit == nil { return 0, fmt.Errorf("SQL row callback is required") }
	...
	for index, row := range result.Rows {
		...
		var value T
		if err := json.Unmarshal(data, &value); err != nil { return index, err }
		if err := visit(value); err != nil { return index + 1, err }
	}
