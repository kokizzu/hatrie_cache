...
- [x] C046b Opt-in low-cardinality dictionary encoding for CH046 columnar wire transfer. `SQLColumnarBlockStreamCompressionDictionary` reduces repeated-string wire bytes while preserving raw v1 as the default; see [CH046_WIRE_DICTIONARY.md](CH046_WIRE_DICTIONARY.md).
...
- [x] M065x Opt-in mutable numeric `RANGE` `FIRST_VALUE`/`LAST_VALUE` maintenance with atomic mutations, affected-partition rebuilds, and a same-position ordered-frame fast path. The append-only default remains unchanged. See [INCREMENTAL_MUTABLE_RANGE_BOUNDARY_WINDOW.md](INCREMENTAL_MUTABLE_RANGE_BOUNDARY_WINDOW.md).
- [x] M065y Opt-in mutable numeric `RANGE` `NTH_VALUE` maintenance with atomic mutations, affected-partition rebuilds, and a same-position peer-aware frame scan. The append-only default remains unchanged. See [INCREMENTAL_MUTABLE_RANGE_NTH_VALUE_WINDOW.md](INCREMENTAL_MUTABLE_RANGE_NTH_VALUE_WINDOW.md).
- [x] M065z Materialize-style same-position mutable rank updates reuse the existing rank output and retained row maps when partition and order keys do not change, avoiding a full affected-partition rebuild while preserving the rebuild fallback for positional mutations. See [INCREMENTAL_RANK_WINDOW.md](INCREMENTAL_RANK_WINDOW.md#same-position-mutable-update-fast-path) and [BENCHMARK.md](BENCHMARK.md#m065z-mutable-rank-arrangement-fast-path).
