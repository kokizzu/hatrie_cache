...
| `NUMBER` / JSON number | `VALUES` or JSON object field | `float64` | Declare `NUMBER`; use `price * 1.1`. | Lua number; use `price * 1.1`. | Declare `NUMBER`; Wasm receives `f64`. | [`TestHatTrieSQLSourceDataTypeMatrix`](sql_production_test.go) |
| `DECIMAL` | `DECIMAL '123.45'`, `CAST(text AS DECIMAL)`, or a typed JSON field | exact `sqlDecimal` text/rational value | No direct UDF ABI: cast explicitly to `NUMBER` (possibly lossy) or `TEXT` before the call. | No direct UDF ABI: cast explicitly to `NUMBER` (possibly lossy) or `TEXT`. | No direct numeric ABI: cast explicitly to `NUMBER` (possibly lossy). | [`TestExecuteSQLQueryPreservesExactDecimalValues`](sql_production_test.go), [`TestHatTrieSQLTypedJSONFieldsValidateAndConvertRows`](sql_production_test.go) |
