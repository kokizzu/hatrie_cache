...
- ClickHouse-style physical columnar `ARRAY JOIN` and `LEFT ARRAY JOIN` for the admitted direct-field shape, with safe fallback for wider SQL: [CH037_COLUMNAR_ARRAY_JOIN.md](CH037_COLUMNAR_ARRAY_JOIN.md), with measurements in [BENCHMARK.md](BENCHMARK.md#ch-037-columnar-array-join)
...
- Opt-in Materialize-style snapshot readiness blocking for multi-source dependents: [M035_SNAPSHOT_BLOCKING.md](M035_SNAPSHOT_BLOCKING.md), with measurements in [BENCHMARK.md#m-u35-snapshot-blocking](BENCHMARK.md#m-u35-snapshot-blocking)
...
- Opt-in pending-plus-running compaction byte admission for bounded maintenance memory: [TT014_COMPACTION_BACKPRESSURE.md](TT014_COMPACTION_BACKPRESSURE.md), with measurements in [BENCHMARK.md](BENCHMARK.md#tt-014-compaction-backpressure)
- Opt-in cache-wide remote read-worker tuning across concurrent prefetch calls: [TT015_READ_WORKER_TUNING.md](TT015_READ_WORKER_TUNING.md), with measurements in [BENCHMARK.md](BENCHMARK.md#tt-015-read-worker-tuning)
...
- ClickHouse-inspired spill runtime join filtering for selective right-heavy joins: [CHU14_RUNTIME_JOIN_FILTER.md](CHU14_RUNTIME_JOIN_FILTER.md), with CPU, memory, and allocation measurements in [BENCHMARK.md](BENCHMARK.md#chu14-spill-runtime-join-filter)
...
- Tarantool-inspired session transaction settings with inherited timeout and read-only guards: [TU05_SESSION_TRANSACTION_SETTINGS.md](TU05_SESSION_TRANSACTION_SETTINGS.md), with measurements in [BENCHMARK.md](BENCHMARK.md#t-u05-session-transaction-settings)
...
- Tarantool-inspired verified incremental snapshot chains with parent-linked manifests and safe content-object retention: [TT008_INCREMENTAL_SNAPSHOT_CHAINS.md](TT008_INCREMENTAL_SNAPSHOT_CHAINS.md), with measurements in [BENCHMARK.md](BENCHMARK.md#tt-008-incremental-snapshot-chains)
- Tarantool-inspired per-space storage-engine selection for independent Pebble/LevelDB paths: [TT012_PER_SPACE_STORAGE.md](TT012_PER_SPACE_STORAGE.md), with lookup measurements in [BENCHMARK.md](BENCHMARK.md#tt-012-per-space-storage-engines)
...
- Opt-in bounded read/write task profiling by table part and column: [C235_TASK_PROFILER.md](C235_TASK_PROFILER.md)
...
- Materialize-style `VALID_AT` partition pruning is available through the
  optional `hatSql.PartitionPruningSourceResolver`; direct literal validity
  predicates become conservative provider hints while the SQL executor still
  rechecks the original predicate. See [MZ009_VALIDITY_PARTITION_PRUNING.md](MZ009_VALIDITY_PARTITION_PRUNING.md)
