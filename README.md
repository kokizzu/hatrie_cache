...
- Per-space memtx versus Vinyl-style storage policy: [T215_PER_SPACE_STORAGE_POLICY.md](T215_PER_SPACE_STORAGE_POLICY.md), with measured selector overhead in [BENCHMARK.md](BENCHMARK.md#t215-per-space-storage-policy).
...
- Mutable R-tree indexes for validated spatial rectangle and point searches: [T220_RTREE_INDEX.md](T220_RTREE_INDEX.md), with selective-query and reusable-buffer measurements in [BENCHMARK.md](BENCHMARK.md#t220-r-tree-spatial-index).
- Typed Roaring-backed BITSET indexes for low-cardinality membership predicates: [T221_BITMAP_INDEX.md](T221_BITMAP_INDEX.md), with streaming-query and build-cost measurements in [BENCHMARK.md](BENCHMARK.md#t221-typed-bitsetbitmap-index).
...
- Round-2 covering-index verification and projection tradeoffs: [T225_COVERING_INDEX.md](T225_COVERING_INDEX.md)
...
- Opt-in SQL memory-overcommit wait queues before query cancellation: [C230_MEMORY_OVERCOMMIT.md](C230_MEMORY_OVERCOMMIT.md), with measurements in [BENCHMARK.md](BENCHMARK.md#c230-memory-overcommit-query-admission)
- EXPLAIN data-skipping mark decisions and rejected-mark reasons: [C236_EXPLAIN_SKIP_DECISIONS.md](C236_EXPLAIN_SKIP_DECISIONS.md), with measurements in [BENCHMARK.md](BENCHMARK.md#c236-explain-data-skipping-decisions)
- EXPLAIN columnar projection selection and estimated read bytes: [C237_EXPLAIN_PROJECTION.md](C237_EXPLAIN_PROJECTION.md), with measurements in [BENCHMARK.md](BENCHMARK.md#c237-explain-projection-selection-and-io-estimate)
- Mutation queue progress counts and elapsed remaining-time estimates: [C238_MUTATION_QUEUE_PROGRESS.md](C238_MUTATION_QUEUE_PROGRESS.md), with measurements in [BENCHMARK.md](BENCHMARK.md#c238-mutation-queue-progress)
- Typed-table patch-part backlog, age, merge accounting, and write amplification: [C239_PART_MERGE_METRICS.md](C239_PART_MERGE_METRICS.md), with measurements in [BENCHMARK.md](BENCHMARK.md#c239-part-merge-metrics)
- Read-only backup and incremental-repository attachment for typed reads and SQL verification without publishing a restore: [C240_READ_ONLY_BACKUP_ATTACHMENT.md](C240_READ_ONLY_BACKUP_ATTACHMENT.md), with measurements in [BENCHMARK.md](BENCHMARK.md#c240-read-only-backup-attachment)
- Fixed-size chunk deduplication for large incremental backup repository files, with a legacy whole-file fallback: [C241_INCREMENTAL_BACKUP_CHUNK_DEDUP.md](C241_INCREMENTAL_BACKUP_CHUNK_DEDUP.md), with measurements in [BENCHMARK.md](BENCHMARK.md#c241-incremental-backup-chunk-deduplication)
- Opt-in per-space synchronous write quorum using longest key-prefix matching for critical records: [T201_PER_SPACE_WRITE_QUORUM.md](T201_PER_SPACE_WRITE_QUORUM.md), with lookup measurements in [BENCHMARK.md](BENCHMARK.md#t201-per-space-write-quorum)
...
- ClickHouse-inspired opt-in parallel hash-join probes: [CH003_PARALLEL_HASH_JOIN.md](CH003_PARALLEL_HASH_JOIN.md), with measurements in [BENCHMARK.md](BENCHMARK.md#ch-g03-parallel-hash-join)
...
- Tarantool-inspired CRC-protected Zstandard archived WAL segments by default: [TTG11_WAL_SEGMENT_COMPRESSION.md](TTG11_WAL_SEGMENT_COMPRESSION.md), with an explicit `SegmentCompressionNone` fallback and measurements in [BENCHMARK.md](BENCHMARK.md#tt-g11-wal-segment-compression)
...
- Opt-in early transaction write-conflict detection with disjoint-key concurrency: [T234_TRANSACTION_CONFLICTS.md](T234_TRANSACTION_CONFLICTS.md), with measurements in [BENCHMARK.md](BENCHMARK.md#t234-early-transaction-conflict-detection)
...
- ClickHouse-inspired opt-in reproducibility hashes combine normalized SQL shape, schema/settings fingerprints, and structural plan steps while excluding runtime-only observations: [CH050_PLAN_REPRODUCIBILITY_HASH.md](CH050_PLAN_REPRODUCIBILITY_HASH.md), with measurements in [BENCHMARK.md](BENCHMARK.md#ch050-plan-reproducibility-hash).
...
- Differential subscription batches consolidate equal full-row updates at the downstream boundary: [M213_DIFFERENTIAL_CONSOLIDATION.md](M213_DIFFERENTIAL_CONSOLIDATION.md), with zero-allocation generated-path measurements in [BENCHMARK.md#m213-differential-batch-consolidation](BENCHMARK.md#m213-differential-batch-consolidation)
