...
- Opt-in ClickHouse-style typed columnar SQL block streaming with projection decoding and bounded repeated-string dictionary encoding: [CH046_NATIVE_WIRE_PROTOCOL.md](CH046_NATIVE_WIRE_PROTOCOL.md), with compression/dictionary tradeoffs in [CH046_WIRE_COMPRESSION.md](CH046_WIRE_COMPRESSION.md)
...
- Opt-in bounded parallel restore of independent content-addressed repository parts: [C242_PARALLEL_RESTORE.md](C242_PARALLEL_RESTORE.md), with CPU, memory, and allocation measurements in [BENCHMARK.md](BENCHMARK.md#c242-bounded-parallel-restore)
...
- Opt-in compaction-debt scheduling for persistent stores, with atomic zero-allocation accounting and retry-safe callbacks: [TR013_COMPACTION_DEBT_SCHEDULER.md](TR013_COMPACTION_DEBT_SCHEDULER.md), with measurements in [BENCHMARK.md](BENCHMARK.md#tr-013-compaction-debt-scheduler)
- Opt-in per-column TTL masking, physical purge, scheduler maintenance, and deadline backup/restore: [CH008_COLUMN_TTL.md](CH008_COLUMN_TTL.md), with measurements in [BENCHMARK.md](BENCHMARK.md#ch-008-column-ttl)
...
- ClickHouse-style `LEFT ARRAY JOIN` preservation for empty and `NULL` arrays: [CH037_LEFT_ARRAY_JOIN.md](CH037_LEFT_ARRAY_JOIN.md), with measurements in [BENCHMARK.md](BENCHMARK.md#ch-037-left-array-join)
...
- Opt-in online space upgrade with per-record versions, target-format writes, lazy conversion, bounded batches, pause/resume, and guarded cutover: [TU20_ONLINE_SPACE_UPGRADE.md](TU20_ONLINE_SPACE_UPGRADE.md), with measured map tradeoffs in [BENCHMARK.md](BENCHMARK.md#t-u20-online-space-upgrade)
- Opt-in Vinyl-style LSM table with bounded memtables, sorted runs, tombstones, compaction, and binary snapshots: [TU17_VINYL_LSM_TABLE.md](TU17_VINYL_LSM_TABLE.md), with measured read/write and snapshot tradeoffs in [BENCHMARK.md](BENCHMARK.md#t-u17-selectable-vinyl-style-lsm-table)
...
- Deterministic, bounded replica bootstrap and join admission with duplicate identity/address checks: [T206_REPLICA_BOOTSTRAP_JOIN.md](T206_REPLICA_BOOTSTRAP_JOIN.md), with control-plane measurements in [BENCHMARK.md](BENCHMARK.md#t206-deterministic-replica-bootstrap-and-join)
- Bounded replica eviction tombstones, generation-fenced rejoin, and stale-state recovery: [T207_REPLICA_EVICTION_RECOVERY.md](T207_REPLICA_EVICTION_RECOVERY.md), with control-plane measurements in [BENCHMARK.md](BENCHMARK.md#t207-replica-eviction-rejoin-and-stale-state-recovery)
- Anonymous read/bootstrap replicas excluded from quorum decisions by an explicit role roster: [T208_ANONYMOUS_REPLICAS.md](T208_ANONYMOUS_REPLICAS.md), with control-plane measurements in [BENCHMARK.md](BENCHMARK.md#t208-anonymous-replicas-and-voter-only-quorum)
- Opt-in ordered replica apply backpressure with bounded burst and sustained rate: [TU37_REPLICA_APPLIER_THROTTLE.md](TU37_REPLICA_APPLIER_THROTTLE.md), with measurements in [BENCHMARK.md](BENCHMARK.md#t209-relayapplier-backpressure)
- Opt-in master-master conflict hooks with source and sequence context: [T210_MASTER_MASTER_CONFLICT_HOOKS.md](T210_MASTER_MASTER_CONFLICT_HOOKS.md), with CPU and allocation measurements in [BENCHMARK.md](BENCHMARK.md#t210-master-master-conflict-hooks)
- Opt-in WAL retention tied to bounded per-replica durable acknowledgments: [T212_WAL_RETENTION_REPLICA_ACKS.md](T212_WAL_RETENTION_REPLICA_ACKS.md), with CPU, memory, and allocation measurements in [BENCHMARK.md](BENCHMARK.md#t212-replica-acknowledgment-wal-retention)
- Opt-in scheduled snapshots with resumable checkpoints and atomic durable manifests: [T213_SCHEDULED_SNAPSHOTS.md](T213_SCHEDULED_SNAPSHOTS.md), with CPU, memory, and allocation measurements in [BENCHMARK.md](BENCHMARK.md#t213-scheduled-snapshots-with-checkpoint-manifests)
- Authenticated streaming snapshots for replicas without a shared filesystem: [T214_STREAMING_SNAPSHOTS.md](T214_STREAMING_SNAPSHOTS.md), with CPU, memory, and allocation measurements in [BENCHMARK.md](BENCHMARK.md#t214-streaming-snapshots-without-a-shared-filesystem)
- Per-space `memtx` or bounded on-disk storage policy with explicit flush/reopen semantics: [T215_PER_SPACE_STORAGE_POLICY.md](T215_PER_SPACE_STORAGE_POLICY.md), with CPU, memory, and allocation measurements in [BENCHMARK.md](BENCHMARK.md#t215-per-space-memtx-versus-on-disk-storage-policy)
- Caller-driven Vinyl-style per-space compaction scheduling and constant-time stale-byte accounting: [T216_STORAGE_COMPACTION_SCHEDULER.md](T216_STORAGE_COMPACTION_SCHEDULER.md), with CPU, memory, and allocation measurements in [BENCHMARK.md](BENCHMARK.md#t216-storage-compaction-scheduler)
- Opt-in typed in-memory columnar spaces with packed booleans, offset-backed strings/bytes, null validity, and atomic append validation: [T217_IN_MEMORY_COLUMNAR_SPACE.md](T217_IN_MEMORY_COLUMNAR_SPACE.md), with CPU, memory, and allocation measurements in [BENCHMARK.md](BENCHMARK.md#t217-in-memory-columnar-space)
- Opt-in multi-part TREE-style ordered indexes with binary-searched prefix/range bounds and k-way merged scans: [T218_MULTI_PART_TREE_INDEX.md](T218_MULTI_PART_TREE_INDEX.md), with publish/build and read-scan measurements in [BENCHMARK.md](BENCHMARK.md#t218-multi-part-tree-index)
- Opt-in immutable packed HASH indexes with flat open addressing, a one-bit occupancy bitmap, and allocation-free exact lookups: [T219_PACKED_HASH_INDEX.md](T219_PACKED_HASH_INDEX.md), with CPU, memory, and allocation measurements in [BENCHMARK.md](BENCHMARK.md#t219-packed-hash-index)
...
- ClickHouse-inspired TTL-driven tuple recompression separate from row deletion: [C246_TTL_RECOMPRESSION.md](C246_TTL_RECOMPRESSION.md), with measurements in [BENCHMARK.md](BENCHMARK.md#c246-ttl-driven-recompression)
...
- Tarantool-inspired opt-in online unique-index validation: [TT025_ONLINE_UNIQUENESS.md](TT025_ONLINE_UNIQUENESS.md), with raw CPU, memory, and allocation tradeoffs in [BENCHMARK.md](BENCHMARK.md#tt-025-online-uniqueness-validation)
...
- ClickHouse-inspired opt-in automatic typed JSON subcolumn promotion: [CH031_AUTOMATIC_TYPED_JSON_SUBCOLUMNS.md](CH031_AUTOMATIC_TYPED_JSON_SUBCOLUMNS.md), with measurements in [BENCHMARK.md](BENCHMARK.md#ch-031-automatic-typed-json-subcolumn-promotion)
- ClickHouse-inspired `HatTrie` integration for bounded dynamic JSON subcolumns: [CH044_JSON_DYNAMIC_SUBCOLUMNS.md](CH044_JSON_DYNAMIC_SUBCOLUMNS.md), with measurements in [BENCHMARK.md](BENCHMARK.md#ch-044-json-dynamic-subcolumns)
...
- ClickHouse-inspired direct columnar append ingestion for typed tables: [CHU22_DIRECT_COLUMNAR_APPEND.md](CHU22_DIRECT_COLUMNAR_APPEND.md), with raw measurements in [BENCHMARK.md](BENCHMARK.md#ch-u22-direct-columnar-append)
- ClickHouse-inspired async-insert queue status and operator flush API: [CHU23_ASYNC_INSERT_QUEUE.md](CHU23_ASYNC_INSERT_QUEUE.md), with raw measurements in [BENCHMARK.md](BENCHMARK.md#ch-u23-async-insert-queue-status-and-flush)
...
- Materialize-inspired bounded schema-drift quarantine: [MZ049_SCHEMA_DRIFT_QUARANTINE.md](MZ049_SCHEMA_DRIFT_QUARANTINE.md), with measurements in [BENCHMARK.md](BENCHMARK.md#mz-049-schema-drift-quarantine)
- Materialize-inspired exactly-once snapshot export with fsynced resume checkpoints: [MZ049_SNAPSHOT_EXPORT.md](MZ049_SNAPSHOT_EXPORT.md), with measurements in [BENCHMARK.md](BENCHMARK.md#mz-049-exactly-once-snapshot-export)
...
- Tarantool-inspired opt-in SQL transaction timeouts with serializable-lock cleanup: [TR038_TRANSACTION_TIMEOUT.md](TR038_TRANSACTION_TIMEOUT.md), with measurements in [BENCHMARK.md](BENCHMARK.md#tr-038-sql-transaction-timeouts)
...
- ClickHouse-inspired per-index skip-index EXPLAIN diagnostics: [CHU49_SKIP_INDEX_EXPLAIN.md](CHU49_SKIP_INDEX_EXPLAIN.md), with measurements in [BENCHMARK.md](BENCHMARK.md#ch-u49-skip-index-explain-diagnostics)
...
- Materialize-inspired temporal join frontier alignment: [M250_TEMPORAL_JOIN_FRONTIER_ALIGNMENT.md](M250_TEMPORAL_JOIN_FRONTIER_ALIGNMENT.md)
...
- Kafka-inspired bounded read-only changefeed offset inspection without creating a consumer: [C249_OFFSET_INSPECTION.md](C249_OFFSET_INSPECTION.md), with measurements in [BENCHMARK.md](BENCHMARK.md#c249-kafka-style-offset-inspection)
- ClickHouse-inspired variant/JSON subcolumn projection that reads only referenced paths: [C248_VARIANT_JSON_SUBCOLUMNS.md](C248_VARIANT_JSON_SUBCOLUMNS.md), with measurements in [BENCHMARK.md](BENCHMARK.md#c248-variantjson-subcolumn-projection)
- ClickHouse-inspired retry-safe async insert identities with optional durable deduplication: [C250_ASYNC_INSERT_IDENTITIES.md](C250_ASYNC_INSERT_IDENTITIES.md), with measurements in [BENCHMARK.md](BENCHMARK.md#c250-retry-safe-async-insert-identities)
...
- ClickHouse-inspired verified immutable-part detach/attach lifecycle with bounded quarantine and streaming file checks: [CH024_DETACH_ATTACH_PARTS.md](CH024_DETACH_ATTACH_PARTS.md), with measurements in [BENCHMARK.md](BENCHMARK.md#ch-024-detach-and-attach-parts)
...
- Materialize-inspired opt-in dynamic dataflow worker scaling: [MZ038_DYNAMIC_DATAFLOW_WORKER_SCALING.md](MZ038_DYNAMIC_DATAFLOW_WORKER_SCALING.md), with measurements in [BENCHMARK.md](BENCHMARK.md#mz-038-dynamic-dataflow-worker-scaling)
...
- Tarantool-inspired bounded SQL row-lock leases for serialized read/mutate workflows: [TT049_ROW_LOCK_LEASES.md](TT049_ROW_LOCK_LEASES.md), with measurements in [BENCHMARK.md](BENCHMARK.md#tt-049-sql-row-lock-leases)
- Opt-in owner-aware SQL row-lock deadlock detection with bounded wait-for graphs: [TR037_DEADLOCK_DETECTION.md](TR037_DEADLOCK_DETECTION.md), with measurements in [BENCHMARK.md](BENCHMARK.md#tr-037-deadlock-detection)
- Optional post-build verification for bounded online SQL index rebuilds: [TR022_INDEX_REBUILD_VERIFICATION.md](TR022_INDEX_REBUILD_VERIFICATION.md), with measurements in [BENCHMARK.md](BENCHMARK.md#tr-022-index-rebuild-verification)
...
- Fixed-width Decimal128/Decimal256 kernels for comparisons, arithmetic, and packed batch filtering: [CHU15_VECTORIZED_DECIMAL_KERNELS.md](CHU15_VECTORIZED_DECIMAL_KERNELS.md), with measurements in [BENCHMARK.md](BENCHMARK.md#ch-u15-fixed-width-decimal-kernels)
...
- Debezium JSON envelope normalization for Kafka table sources: use `NewDebeziumKafkaTableDecoder` to map create/read/update/delete events, structured keys, schema/payload envelopes, and tombstones into the existing source contract; see [MZ013_DEBEZIUM_KAFKA_DECODER.md](MZ013_DEBEZIUM_KAFKA_DECODER.md).
- Durable Kafka source checkpoints: use `ApplyBatchWithCheckpoint`, `RestoreCheckpoint`, or `ConsumeOnceWithCheckpoint` to couple rows, offsets, and replay markers before broker offset commit; see [MZ016_KAFKA_SOURCE_CHECKPOINT.md](MZ016_KAFKA_SOURCE_CHECKPOINT.md).
- Durable Kafka connector lifecycle state: use `PauseWithCheckpoint` and `ResumeWithCheckpoint` to persist operator pause/resume decisions with the source checkpoint; see [MZ019_KAFKA_SOURCE_LIFECYCLE.md](MZ019_KAFKA_SOURCE_LIFECYCLE.md).
- Opt-in two-phase sink progress checkpoints: use `SQLSinkTwoPhaseCoordinator` to persist prepared state before publication and committed progress after an idempotent sink commit; see [MZ020_TWO_PHASE_SINK_CHECKPOINT.md](MZ020_TWO_PHASE_SINK_CHECKPOINT.md) and [BENCHMARK.md](BENCHMARK.md#mz-020-two-phase-sink-progress-checkpoints).
- Deterministic sink idempotency tokens: use `DeriveSQLSinkIdempotencyKey` or `DeriveSQLSinkCommitIdempotencyKey` so retries with regenerated transaction IDs deduplicate by sink frontier and exact batch bytes; see [MZ021_SINK_IDEMPOTENCY_TOKENS.md](MZ021_SINK_IDEMPOTENCY_TOKENS.md) and [BENCHMARK.md](BENCHMARK.md#mz-021-sink-idempotency-tokens).
- Confluent Avro schema-ID resolution for Kafka table sources: use the bounded `AvroSchemaRegistry` and inject the actual Avro decoder; see [MZ014_AVRO_SCHEMA_REGISTRY.md](MZ014_AVRO_SCHEMA_REGISTRY.md).
- Confluent Protobuf schema-ID and nested message-index resolution for Kafka table sources: use the bounded `ProtobufSchemaRegistry` and inject the dynamic-message decoder; see [MZ015_PROTOBUF_SCHEMA_REGISTRY.md](MZ015_PROTOBUF_SCHEMA_REGISTRY.md).
...
- Durable source-versioned SQL planner statistics: call `SaveSQLPlannerStatistics` and `LoadSQLPlannerStatistics` for explicit, source-validated `HPS1` recovery metadata. See [SQL_PLANNER_STATISTICS.md](SQL_PLANNER_STATISTICS.md).
- Opt-in automatic SQL index choice with explainable candidate rejection and adaptive fallback: [TR031_AUTOMATIC_INDEX_SELECTION.md](TR031_AUTOMATIC_INDEX_SELECTION.md), with measurements in [BENCHMARK.md](BENCHMARK.md#tr-31-automatic-index-choice).
...
- Incremental join arrangement selection across explicit exact predicates: [MU045_JOIN_ARRANGEMENT_SELECTION.md](MU045_JOIN_ARRANGEMENT_SELECTION.md), with measurements in [BENCHMARK.md](BENCHMARK.md#m-u45-incremental-join-arrangement-selection)
- Source connector health remediation with bounded retries and quarantine: [MU048_CONNECTOR_HEALTH_REMEDIATION.md](MU048_CONNECTOR_HEALTH_REMEDIATION.md), with measurements in [BENCHMARK.md](BENCHMARK.md#m-u48-source-connector-health-remediation)
...
Configured SQL JSON index sources also use a monotonic write generation for
freshness, replacing repeated full-document equality checks in indexed read,
maintenance, ordering, and statistics paths. On separate but identical 1 MiB
JSON strings, the check is 38,563x faster with zero allocations; the counter
...
nested values retain the prior whole-payload path. A single trailing nested whole-sequence value
receives the same direct validation without changing the
fallback scan for earlier nested values; complete slice replacements improve
1.41x/2.02x and priority-queue replacements improve 1.44x/1.76x; see the
...
Public checked Bloom, Cuckoo, and Count-Min Sketch lookups now hash ordinary
canonical strings directly instead of allocating an `encoding/json` key. Their
measured hit paths are 1.91x-2.33x faster and allocation-free; escaped and
structured controls retain the prior encoder with neutral memory and CPU. XOR
...
replaced.
The follow-up [linear expiration-index rebuild](BENCHMARK.md#linear-expiration-index-rebuild)
...
For data ordered by multiple numeric keys, set `SparsePrimaryFields` in
lexicographic order, for example `[]string{"tenant", "id"}`. The composite
marks are also opt-in, require a nondecreasing tuple stream, and safely fall
back when the configured fields are incomplete or out of order. See
[CHU18_COMPOSITE_PRIMARY_MARK_PRUNING.md](CHU18_COMPOSITE_PRIMARY_MARK_PRUNING.md)
for the configuration contract and measured tradeoff.
...
Background compaction can optionally pace estimated read/write bytes with
...
- Optional nested atomic transaction scopes are available through `SQLTransaction.Scope` and `SQLTransaction.BeginScope`; callback failures roll back only the nested boundary while the parent transaction remains usable. See [TT032_ATOMIC_TRANSACTION_SCOPES.md](TT032_ATOMIC_TRANSACTION_SCOPES.md) and the measurements in [BENCHMARK.md](BENCHMARK.md#t032-atomic-transaction-scopes).
- Optional cooperative MVCC safe points are available through `SQLTransaction.Yield(ctx)`; the private snapshot transaction can yield without holding its mutex across the scheduler handoff. See [TT033_MVCC_COOPERATIVE_YIELD.md](TT033_MVCC_COOPERATIVE_YIELD.md) and the measurements in [BENCHMARK.md](BENCHMARK.md#t033-mvcc-cooperative-yield).
- Optional early transaction conflict detection is available through `SQLTransactionOptions.EarlyConflictDetection`; stale staged writes abort before publishing private work while the default remains commit-time-only detection. See [TT034_EARLY_TRANSACTION_CONFLICTS.md](TT034_EARLY_TRANSACTION_CONFLICTS.md) and the measurements in [BENCHMARK.md](BENCHMARK.md#t034-early-transaction-conflict-detection).
