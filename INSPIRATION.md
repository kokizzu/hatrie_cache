...
- [x] T047 Synchronous replication with an explicit quorum. The public single-command path and atomic `BATCH` quorum semantics are implemented and tested through opt-in `MonitoringOptions.WriteQuorum` / `CacheGRPCOptions.WriteQuorum`. Partial: transport-neutral `ExecuteClusterWriteCommitWithLedger` now adds bounded, CRC-checked, atomically replaced durable participant outcome state and rejects blind replay; authenticated transport wiring, participant-side durable writes, and automatic reconciliation remain caller-owned/open.
