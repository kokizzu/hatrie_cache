...
- [x] C015b SQL packed numeric predicate kernel. Direct numeric columnar filters reuse fixed-width bytes and validity bits without per-row interface boxing, while legacy and unsupported shapes retain fallback semantics.
...
- [x] C154f Durable schema migration barrier snapshots with strict validation,
...
- [x] M032e Opt-in distributed frontier coordination across independent barrier groups, with exact-frontier snapshot binding; source transport, membership, and consensus remain caller-owned. See [M032_DISTRIBUTED_FRONTIER.md](M032_DISTRIBUTED_FRONTIER.md).
- [ ] M033 Timestamp oracle for globally ordered writes.
- [x] M034 Epoch fencing for leases across process restarts. `hatDataStructure.VisibilityQueue` keeps the legacy raw-ID path unchanged and provides `NewVisibilityQueueWithEpoch`, `LeaseWithToken`, `AckToken`, and `NackToken` so persisted or transferred leases include an explicit restart epoch; see [VISIBILITY_QUEUE.md](VISIBILITY_QUEUE.md).
...
- [x] M037i Signed differential int64 grouped `SUM(DISTINCT)` maintenance keeps
  ...
  [DIFFERENTIAL_GROUP_BY.md](DIFFERENTIAL_GROUP_BY.md).
- [x] M037j Signed differential int64 grouped `COUNT(DISTINCT)` maintenance
...
- [x] M052e Extend the opt-in built-in executor to one-direct-integer-key
  grouped `CACHE`/`KEYS` batches with built-in aggregates, first-seen group
  order, signed/unsigned integer and `NULL` keys, and explicit rejection of
...
- [x] M052g Extend the opt-in built-in executor to finite `LIMIT`/`OFFSET`
  windows for scalar filter/project batches. The native path stops after the
  post-filter page while grouped, aggregate, and `DISTINCT` windows remain
...
- [x] M052i Extend the native ordered page path to multiple direct source-field
  keys with mixed direction, stable ties, and NULL ordering. The retained
  implementation uses the existing comparator and bounded candidate heap;
  stack-backed key scratch was measured and reverted because it increased heap
  and runtime without reducing allocations. The paired benchmark is 4.34x
  faster with 16.16x fewer bytes and 4.35x fewer allocations; see
  [BENCHMARK.md](BENCHMARK.md#native-sql-dataflow-composite-ordered-limit).
- [x] M052j Extend the opt-in native executor to grouped ordered pages. Compact
  grouped `COUNT`/`SUM` state is followed by a bounded Top-N heap over selected
  output aliases, preserving mixed ordering, stable ties, NULL semantics, and
  finite `LIMIT`/`OFFSET`; unsupported grouped ordering remains fail-closed.
...
- [x] M052m Extend native `DISTINCT` dataflow to direct string keys. A lazy
  string membership index preserves first-seen order, `NULL` and empty-string
...
- [x] M052n Extend native `DISTINCT` dataflow to two direct typed fields. A
  ...
  remain fail-closed. The paired benchmark is 3.43x faster with 5.00x fewer
  bytes and 3.91x fewer allocations; see
...
- [x] M064f Importable bounded recursive worklist primitive with deterministic FIFO processing, duplicate-key coalescing, cancellation, and step/pending limits; SQL planner wiring remains open. See [MZ036_FIXPOINT_SCHEDULER.md](MZ036_FIXPOINT_SCHEDULER.md).
...
- [x] M071 Materialize-style session snapshot consistency tokens. `hatSql.SQLSnapshotTokenCodec` carries a bounded HMAC-authenticated frontier across related queries and reuses the existing `AsOfFrontier` provider path; empty tokens preserve the default path. See [MZ044_SNAPSHOT_TOKENS.md](MZ044_SNAPSHOT_TOKENS.md) and [BENCHMARK.md](BENCHMARK.md#mz-044-session-snapshot-tokens).
...
- [x] M090c Projected materialized source resolution. `hatSql.ProjectedSourceResolver`
  lets compute nodes request only fields needed by conservative single-source
  queries, including the automatic native scalar path, while unsupported
...
- [x] T015a Vinyl-style independent read-worker tuning is available as opt-in `RemotePartCacheOptions.MaxPrefetchConcurrency`; it caps remote loader calls across concurrent prefetch calls while preserving the existing per-call cap. See [TT015_READ_WORKER_TUNING.md](TT015_READ_WORKER_TUNING.md).
...
- [x] MZ016 reconciled existing schema-evolution support. Safe additive rolling changes, bounded source-version admission/rollback, and coordinated activation are already implemented; destructive live drops remain rejected by design. See [MU02_CONNECTOR_SCHEMA_EVOLUTION.md](MU02_CONNECTOR_SCHEMA_EVOLUTION.md).
...
- [x] T095d Exact-capacity posting-list materialization. Shared hash and functional-index ID lookups reserve the known posting cardinality before materializing a fresh result, removing append growth allocations without changing posting order or reusable-scratch behavior. The measured fresh 100-ID lookup is 1.18x faster, with 3 to 1 allocations and 920 to 896 B/op. See [BENCHMARK.md](BENCHMARK.md#posting-list-exact-capacity).
- [x] T095e Monotonic secondary-index posting insertion. Sorted posting lists append directly when the incoming stable ID is beyond the current tail, while arbitrary insertion and duplicate handling retain the existing binary-search path. The isolated 10,000-ID build is 1.92x faster and the integrated hash-index build is 1.07x faster with unchanged allocation counts. See [BENCHMARK.md](BENCHMARK.md#monotonic-posting-insertion).
- [x] T095f Sorted posting removal boundary checks. Secondary-index deletion returns immediately for IDs above the tail and removes an existing tail directly, while middle lookups retain binary search. High-miss removal is 4.28x faster and tail removal is 2.07x faster with zero allocations; the middle control is unchanged within benchmark noise. See [BENCHMARK.md](BENCHMARK.md#sorted-posting-removal-boundaries).
- [x] T095g R-tree small-result sort fast paths. Spatial search skips sorting for zero or one hit and compare-swaps two hits directly, while larger result sets retain the full ascending sort. The measured empty, one-hit, two-hit, and many-hit cases are 1.30x, 1.04x, 1.15x, and 1.02x faster with unchanged memory. See [BENCHMARK.md](BENCHMARK.md#rtree-small-result-sort-fast-paths).
- [x] T095h Ordered snapshot-cursor seek fast paths. Forward and reverse snapshot cursors compare directly for one or two entries and retain binary search for larger vectors. The changed one- and two-entry probes are 1.06x-1.29x faster with unchanged zero-allocation behavior; ordinary iterator seeks are unchanged. See [BENCHMARK.md](BENCHMARK.md#ordered-snapshot-cursor-seek-fast-paths).
- [x] T095i Order-preserving ordered-index upserts. A comparator-equivalent replacement updates its entry in place when no snapshot is active, avoiding vector shifts and position-map rewrites; active snapshots retain copy-on-write. The measured same-key workload is 31.9x-9,236x faster from 64 to 10,000 entries with unchanged zero-allocation behavior. See [BENCHMARK.md](BENCHMARK.md#ordered-index-same-key-upsert-fast-path).
- [x] T095j Monotonic ordered-index append fast path. A new `(key, ID)` pair after the current tail appends directly, while arbitrary insertion retains binary search and copy behavior. The measured monotonic build is 1.21x-2.00x faster from 64 to 10,000 entries with unchanged allocation counts and effectively unchanged memory. See [BENCHMARK.md](BENCHMARK.md#ordered-index-monotonic-append-fast-path).
...
- [x] T150 Language-neutral client SDK coverage. `hat/hatMonitoring.Client.Command` and `Client.Batch` expose the authenticated JSON command contract for non-gRPC callers, while `CommandWithFormat`, `BatchWithFormat`, and `Client.CommandWireFormat` provide opt-in protobuf transfer; the server and JSON default runtime behavior are unchanged.
...
- [x] M065y opt-in mutable numeric `RANGE` `NTH_VALUE` maintenance with
  atomic mutations, affected-partition rebuilds, and a same-position
  peer-aware frame scan. The append-only default remains unchanged; see
...
- [x] M065z Materialize-style same-position mutable rank updates reuse the
  existing rank output and retained row maps when partition and order keys do
  not change, avoiding a full affected-partition rebuild while preserving the
