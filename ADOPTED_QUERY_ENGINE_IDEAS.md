...
| ClickHouse | Grouping-set branch plan sharing | Adopted as a plan-construction optimization | `GROUPING SETS`, `ROLLUP`, and `CUBE` branches share immutable source and relational descriptors while cloning only rewrite-local expressions. The execution model remains branch-per-set; native one-pass aggregation is still open. [CH041_GROUPING_PLAN_SHARING.md](CH041_GROUPING_PLAN_SHARING.md) |
