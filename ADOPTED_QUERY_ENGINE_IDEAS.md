...
| --- | --- | --- | --- |
| Materialize | Reusable maintained-result cache for identical read expressions | Adopted as an opt-in bounded in-flight cache | NewMaintainedResultCache coalesces concurrent versioned reads, shares owner errors, retries when freshness changes, and preserves dependency invalidation; the default cache path remains unchanged. See [M248_MAINTAINED_RESULT_CACHE.md](M248_MAINTAINED_RESULT_CACHE.md) and [BENCHMARK.md#m248-maintained-result-cache](BENCHMARK.md#m248-maintained-result-cache). |
| Materialize | Temporal join frontier alignment | Adopted as an opt-in paired frontier barrier | `SQLTemporalJoinFrontierAlignment` waits for both input barriers at one requested timestamp and returns the two observed frontiers plus their safe minimum; ordinary barriers and SQL query execution remain unchanged. See [M250_TEMPORAL_JOIN_FRONTIER_ALIGNMENT.md](M250_TEMPORAL_JOIN_FRONTIER_ALIGNMENT.md) and [BENCHMARK.md#m250-temporal-join-frontier-alignment](BENCHMARK.md#m250-temporal-join-frontier-alignment). |
| Tarantool | Per-space synchronous replication quorum | Adopted as an opt-in critical-space policy registry | `hatReplication.PerSpaceWriteQuorum` routes only configured spaces through the existing proposal-bound journal quorum; unconfigured spaces report `Enforced=false` without invoking the acknowledgement callback. See [T201_PER_SPACE_SYNC_QUORUM.md](T201_PER_SPACE_SYNC_QUORUM.md) and [BENCHMARK.md#t201-per-space-synchronous-replication-quorum](BENCHMARK.md#t201-per-space-synchronous-replication-quorum). |
| Tarantool | Automatic leader election for a replica set | Adopted as an opt-in, transport-neutral election state machine | `hatReplication.ReplicaSetLeaderElection` tracks fixed voters, rejects stale heartbeats, requires quorum, chooses the freshest caught-up voter deterministically, and fences commit with term, generation, and fencing token. It does not start a goroutine or perform network I/O; callers own authenticated heartbeats and transport. See [T202_REPLICA_SET_LEADER_ELECTION.md](T202_REPLICA_SET_LEADER_ELECTION.md) and [BENCHMARK.md#t202-replica-set-leader-election](BENCHMARK.md#t202-replica-set-leader-election). |
| Tarantool | Strict leader fencing against stale writers after failover | Adopted as an opt-in local write gate | `hatReplication.ReplicaSetLeaderWriteFence` admits a callback only when node, term, and fencing token exactly match the committed leader; `Advance` requires both term and token to increase and serializes the transition with admitted callbacks. See [T203_LEADER_WRITE_FENCE.md](T203_LEADER_WRITE_FENCE.md) and [BENCHMARK.md#t203-strict-leader-write-fencing](BENCHMARK.md#t203-strict-leader-write-fencing). |
| Tarantool | Supervised failover with explicit operator override and recovery state | Adopted as an opt-in, operator-gated lifecycle | `hatReplication.SupervisedFailoverCoordinator` requires named approval or an auditable override before commit, tracks recovery success/failure, requires the exact candidate and fencing token to report caught-up recovery, and requires an explicit reset. See [T204_SUPERVISED_FAILOVER.md](T204_SUPERVISED_FAILOVER.md) and [BENCHMARK.md#t204-supervised-failover-lifecycle](BENCHMARK.md#t204-supervised-failover-lifecycle). |
| ClickHouse / Materialize / Tarantool | LSN-based replication lag and apply-throughput metrics | Adopted as an opt-in bounded collector | `hatReplication.ReplicationProgressMetrics` retains at most a configured target count, tracks a global source-LSN high-water mark, rejects regressing observations, and reports detached sorted lag/LSN-rate/byte-rate snapshots. It is not wired into transport by default. See [T205_REPLICATION_PROGRESS_METRICS.md](T205_REPLICATION_PROGRESS_METRICS.md) and [BENCHMARK.md#t205-replication-progress-metrics](BENCHMARK.md#t205-replication-progress-metrics). |
| ClickHouse / Materialize / Tarantool | Deterministic replica bootstrap and join admission | Adopted as a bounded control-plane registry | `hatReplication.ReplicaJoinAdmission` normalizes node/address identities, rejects duplicate active or pending names/endpoints, chooses the healthiest highest-applied source deterministically, and commits only an exact active bootstrap plan at the next topology generation. It does not perform network or storage side effects. See [T206_REPLICA_BOOTSTRAP_JOIN.md](T206_REPLICA_BOOTSTRAP_JOIN.md) and [BENCHMARK.md#t206-deterministic-replica-bootstrap-and-join](BENCHMARK.md#t206-deterministic-replica-bootstrap-and-join). |
| ClickHouse / Materialize / Tarantool | Anonymous replicas excluded from quorum decisions | Adopted as an explicit voter-default role and quorum boundary | `hatReplication.ReplicaRoleAnonymous`, `ReplicaJoinAdmission.RoleRoster`, and `ExecuteVoterWriteQuorum` keep read/bootstrap replicas out of voter rosters and reject anonymous or unknown quorum targets before callbacks. See [T208_ANONYMOUS_REPLICAS.md](T208_ANONYMOUS_REPLICAS.md) and [BENCHMARK.md#t208-anonymous-replicas-and-voter-only-quorum](BENCHMARK.md#t208-anonymous-replicas-and-voter-only-quorum). |
| ClickHouse / Materialize / Tarantool | Replica eviction, rejoin, and stale-state recovery | Adopted as a bounded control-plane tombstone protocol | `hatReplication.ReplicaJoinAdmission.Evict`, `PrepareRejoin`, and `CommitRejoin` retain monotone eviction epochs, fence ordinary joins for evicted identities, require exact caught-up bootstrap state, and reject stale topology generations. See [T207_REPLICA_EVICTION_RECOVERY.md](T207_REPLICA_EVICTION_RECOVERY.md) and [BENCHMARK.md#t207-replica-eviction-rejoin-and-stale-state-recovery](BENCHMARK.md#t207-replica-eviction-rejoin-and-stale-state-recovery). |
| Tarantool | Relay/applier backpressure for lagging replicas | Adopted as an opt-in local apply throttle | `hatReplication.ApplierThrottle` reserves ordered batch capacity with a bounded burst and sustained rate, while the default nil option preserves unthrottled behavior. See [TU37_REPLICA_APPLIER_THROTTLE.md](TU37_REPLICA_APPLIER_THROTTLE.md) and [BENCHMARK.md#t209-relayapplier-backpressure](BENCHMARK.md#t209-relayapplier-backpressure). |
| Tarantool | Master-master conflict hooks with source and sequence context | Adopted as an opt-in conflict decision callback | `hatReplication.ConflictPolicy.Hook` receives the space and both validated `ConflictVersion` values, exposes source IDs and source sequences without raw values, and can choose left/right, delegate to the configured policy, or reject. See [T210_MASTER_MASTER_CONFLICT_HOOKS.md](T210_MASTER_MASTER_CONFLICT_HOOKS.md) and [BENCHMARK.md#t210-master-master-conflict-hooks](BENCHMARK.md#t210-master-master-conflict-hooks). |
| Tarantool | Configurable WAL synchronization modes with durability reporting | Adopted as an opt-in bounded per-space policy registry | `hatJournal.SpaceSyncPolicyRegistry` resolves periodic, immediate, or disabled WAL-sync requirements with explicit `Durable` and `Immediate` reporting; the journal owner retains append/fsync/recovery responsibility. See [TU34_SPACE_WAL_SYNC_POLICY.md](TU34_SPACE_WAL_SYNC_POLICY.md) and [BENCHMARK.md#t-u34-per-space-wal-sync-policy-registry](BENCHMARK.md#t-u34-per-space-wal-sync-policy-registry). |
| ClickHouse / Materialize / Tarantool | WAL retention and rotation tied to replica acknowledgments | Adopted as an opt-in bounded acknowledgment floor | `CommandJournalOptions.ReplicaRetentionCapacity` and `AcknowledgeReplicaThrough` retain history until every registered replica has acknowledged it; reopened enabled journals stay protected until fresh acknowledgments arrive, while capacity `0` preserves the default path. See [T212_WAL_RETENTION_REPLICA_ACKS.md](T212_WAL_RETENTION_REPLICA_ACKS.md) and [BENCHMARK.md#t212-replica-acknowledgment-wal-retention](BENCHMARK.md#t212-replica-acknowledgment-wal-retention). |
| ClickHouse / Materialize / Tarantool | Scheduled snapshots with checkpoint manifests and atomic publication | Adopted as an opt-in scheduler over resumable export | `hatCache.ScheduledSnapshotter` runs serialized periodic exports, keeps failed-copy checkpoints for retry, and publishes a validated digest/journal-coordinate manifest only after the snapshot target is atomically renamed. See [T213_SCHEDULED_SNAPSHOTS.md](T213_SCHEDULED_SNAPSHOTS.md) and [BENCHMARK.md#t213-scheduled-snapshots-with-checkpoint-manifests](BENCHMARK.md#t213-scheduled-snapshots-with-checkpoint-manifests). |
| ClickHouse / Materialize / Tarantool | Streaming snapshots without a shared filesystem | Adopted as an authenticated direct-writer transfer path | `StreamCommandJournalSnapshot` forwards the existing snapshot bytes without a local temporary file, validates sequence/format/length/SHA-256 response metadata, and leaves final remote commit semantics to the receiving writer. See [T214_STREAMING_SNAPSHOTS.md](T214_STREAMING_SNAPSHOTS.md) and [BENCHMARK.md#t214-streaming-snapshots-without-a-shared-filesystem](BENCHMARK.md#t214-streaming-snapshots-without-a-shared-filesystem). |
| Tarantool | Per-space memtx versus on-disk storage policy | Adopted as an opt-in importable storage-space boundary | `hatDataStructure.StorageSpace` keeps the default policy in memory and delegates bounded on-disk storage, CRC recovery, flush, reopen, and compaction to `SpillableArrangement`. See [T215_PER_SPACE_STORAGE_POLICY.md](T215_PER_SPACE_STORAGE_POLICY.md) and [BENCHMARK.md#t215-per-space-memtx-versus-on-disk-storage-policy](BENCHMARK.md#t215-per-space-memtx-versus-on-disk-storage-policy). |
| ClickHouse / Tarantool | Vinyl-style per-space compaction debt scheduling | Adopted as a caller-driven, default-off scheduler | `StorageSpaceCompactionScheduler` selects on-disk spaces by maintained stale-byte debt, bounds each run by `MaxSpacesPerRun`, skips memtx spaces, honors cancellation, and reports reclaimed bytes. See [T216_STORAGE_COMPACTION_SCHEDULER.md](T216_STORAGE_COMPACTION_SCHEDULER.md) and [BENCHMARK.md#t216-storage-compaction-scheduler](BENCHMARK.md#t216-storage-compaction-scheduler). |
| ClickHouse / Materialize / Tarantool | In-memory typed columnar analytical space | Adopted as an opt-in importable storage primitive | `ColumnarSpace` uses fixed-width, packed-bit, and offset-backed buffers with lazy null validity, atomic append validation, detached snapshots, and bounded capacity; existing row and SQL paths remain unchanged. See [T217_IN_MEMORY_COLUMNAR_SPACE.md](T217_IN_MEMORY_COLUMNAR_SPACE.md) and [BENCHMARK.md#t217-in-memory-columnar-space](BENCHMARK.md#t217-in-memory-columnar-space). |
| ClickHouse / Materialize / Tarantool | Multi-part TREE-style ordered runs | Adopted as an opt-in importable index primitive | `MultiPartTreeIndex` copies and sorts immutable parts, binary-searches inclusive bounds in every part, and k-way merges the matching entries with deterministic key/ID ordering. It improves batch publication/build cost but has a slower, allocation-bearing merged scan than a single `OrderedIndex`; existing SQL, persistence, and replication paths remain unchanged. See [T218_MULTI_PART_TREE_INDEX.md](T218_MULTI_PART_TREE_INDEX.md) and [BENCHMARK.md#t218-multi-part-tree-index](BENCHMARK.md#t218-multi-part-tree-index). |
| Tarantool / ClickHouse | Packed HASH primary/exact-match table | Adopted as an opt-in importable index primitive | `PackedHashIndex` copies unique-key entries into a flat open-addressed table with a one-bit occupancy bitmap, stable caller-supplied hash, and allocation-free lock-free reads. It complements rather than replaces the mutable map-backed `HashIndex`; SQL, persistence, replication, backup, and wire paths remain unchanged. See [T219_PACKED_HASH_INDEX.md](T219_PACKED_HASH_INDEX.md) and [BENCHMARK.md#t219-packed-hash-index](BENCHMARK.md#t219-packed-hash-index). |
| Materialize | Per-operator update, batch, and frontier metrics | Adopted as an opt-in observer sidecar | `SQLQueryOptions.OperatorMetrics` adds `QueryEvent.OperatorMetrics` without changing the legacy `Operators` allocation shape. `AsOfFrontier` is reported only when exact; live queries omit unknown frontiers. See [M242_OPERATOR_METRICS.md](M242_OPERATOR_METRICS.md) and [BENCHMARK.md#m242-per-operator-update-batch-and-frontier-metrics](BENCHMARK.md#m242-per-operator-update-batch-and-frontier-metrics). |
| Materialize | Arrangement memory split by key, value, and trace history | Adopted for typed-table aggregate arrangements | `TypedTableAggregateArrangementStats` reports `EstimatedKeyBytes`, `EstimatedValueBytes`, and `EstimatedTraceBytes`; the existing `EstimatedBytes` total remains exact and the source changelog is not counted as trace state. See [M243_ARRANGEMENT_MEMORY_METRICS.md](M243_ARRANGEMENT_MEMORY_METRICS.md) and [BENCHMARK.md#m243-arrangement-memory-metrics](BENCHMARK.md#m243-arrangement-memory-metrics). |
| Materialize | Compaction debt measured against the current logical frontier | Adopted for typed-table aggregate arrangements | `TypedTableAggregateArrangementStats.CompactionDebt` reports the nonnegative `SourceSequence - CompactedThrough` distance, independent of arrangement checkpoint lag. See [M244_COMPACTION_DEBT.md](M244_COMPACTION_DEBT.md) and [BENCHMARK.md#m244-compaction-debt-at-the-logical-frontier](BENCHMARK.md#m244-compaction-debt-at-the-logical-frontier). |
| Materialize | Timestamp throughput and input-to-output freshness latency | Adopted as an opt-in telemetry observation API | `SQLTelemetry.ObserveSQLTimestamp` records update/batch counters, monotone logical input/output timestamps, and guarded wall-clock freshness latency without changing query execution. See [M245_TIMESTAMP_TELEMETRY.md](M245_TIMESTAMP_TELEMETRY.md) and [BENCHMARK.md#m245-timestamp-throughput-and-input-to-output-latency](BENCHMARK.md#m245-timestamp-throughput-and-input-to-output-latency). |
| Materialize / ClickHouse | Per-object history-retention policies and bounded storage accounting | Adopted as an opt-in caller-driven frontier policy registry | `FrontierRetentionRegistry.SetPolicy` and `SetUsage` track per-frontier logical-history and estimated-byte budgets without changing the existing snapshot/compaction path; `PolicySnapshot` exposes budget status while `SafeCompactionBefore` remains authoritative. See [M246_FRONTIER_RETENTION_POLICY.md](M246_FRONTIER_RETENTION_POLICY.md) and [BENCHMARK.md#m246-per-object-history-retention-policies](BENCHMARK.md#m246-per-object-history-retention-policies). |
| Materialize | Resume errors that identify expired frontiers | Adopted as a backwards-compatible typed error | `FrontierRetentionExpiredError` carries frontier ID, requested as-of, and observed bounds while unwrapping to `ErrFrontierRetentionExpired`; valid checks and ordinary compaction behavior remain unchanged. See [M247_FRONTIER_EXPIRY_ERRORS.md](M247_FRONTIER_EXPIRY_ERRORS.md) and [BENCHMARK.md#m247-frontier-expiry-errors](BENCHMARK.md#m247-frontier-expiry-errors). |
...
| T233 | Tarantool | MVCC cooperative yields | Importable snapshot transactions expose cancellation-aware safe points without default-path overhead | [TT033_MVCC_COOPERATIVE_YIELD.md](TT033_MVCC_COOPERATIVE_YIELD.md) |
| T234 | Tarantool | early transaction conflict detection | Opt-in snapshot transactions reject competing live mutations before and after staged writes while retaining commit-time validation | [TT034_EARLY_TRANSACTION_CONFLICTS.md](TT034_EARLY_TRANSACTION_CONFLICTS.md) |
| T235 | Tarantool | cooperative fiber workers | Importable opt-in `hatFiber.Scheduler` runs bounded stackless steps with cancellation, generation-safe reaping, and typed fiber synchronization | [TU30_COOPERATIVE_FIBER_SCHEDULER.md](TU30_COOPERATIVE_FIBER_SCHEDULER.md) |
