...
| ClickHouse | Typed scalar JSON subcolumns | Adopted as an opt-in narrow columnar optimization, including M046 bounded Top-N | `ColumnarJSONSubcolumn` stores typed scalar JSON paths with compact payloads plus presence/validity bitmaps. `ColumnarJSONSubcolumnSourceResolver` can serve them for literal `JSON_VALUE`, `JSON_EXISTS`, and `JSON_QUERY` paths; M046 also uses an available scalar path for bounded `ORDER BY JSON_VALUE(...) LIMIT/OFFSET`, while unsupported or unavailable cases preserve the existing map/row path. The measured 4,096-row filter is 3.92x faster, and the bounded Top-N fixture is 70.4x faster with 1,454x lower heap and 288x fewer allocations. See [CH031_TYPED_JSON_SUBCOLUMNS.md](CH031_TYPED_JSON_SUBCOLUMNS.md), [M046_JSON_SUBCOLUMN_TOPN.md](M046_JSON_SUBCOLUMN_TOPN.md), and [BENCHMARK.md](BENCHMARK.md#ch-031-typed-json-subcolumns). |
