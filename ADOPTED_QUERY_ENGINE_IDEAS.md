...
| ClickHouse/Materialize | Ordered prefix filtering | Adopted as an explicit SQL index path | `LIKE 'prefix%'` on a binary-collation direct JSON string field reuses the existing ordered field index with a binary search and final predicate recheck. Mixed types, wildcard patterns, missing indexes, and admission denial retain the ordinary scan; no new default or per-row metadata is added. [SQL_LIKE_PREFIX_INDEX.md](SQL_LIKE_PREFIX_INDEX.md) |
...
| JSON `LIKE 'prefix%'`, 10,000-row source with 5% matches | Median 0.238 ms, 261,852 B, and 2,530 allocations through the ordered prefix index versus 13.734 ms, 9,922,038 B, and 160,048 allocations on the scan baseline: 57.7x faster, 37.9x less allocated heap, and 63.3x fewer allocations. The index is explicit and mixed or complex patterns fall back. See [BENCHMARK.md](BENCHMARK.md#json-like-prefix-index). |
