...
| Tarantool | IProto prepared response schemas | Adopted as an opt-in compact-peer schema negotiation extension | `CompactPeerSession.CallTemplate` can carry an opaque response schema ID, validates the echoed ID, and preserves the legacy frame/allocation path when disabled. See [TR043C_COMPACT_PEER_RESPONSE_SCHEMA.md](TR043C_COMPACT_PEER_RESPONSE_SCHEMA.md) and [BENCHMARK.md#tr-043c-compact-peer-response-schemas](BENCHMARK.md#tr-043c-compact-peer-response-schemas). |
...
| ClickHouse/Materialize | Code-keyed columnar dictionary grouping | Adopted automatically when a trusted dictionary is already present | Binary-collated single-level and opt-in two-level columnar vector `GROUP BY` uses `uint32` dictionary codes for state lookup and resolves each group value once; Unicode collation, untrusted dictionaries, malformed layouts, and unsupported shapes retain the existing executor. See [MZ026_COLUMNAR_DICTIONARY_GROUP.md](MZ026_COLUMNAR_DICTIONARY_GROUP.md) and [BENCHMARK.md](BENCHMARK.md#mz-026-columnar-dictionary-group-keys). |
| Materialize | Exact incremental multiset maintenance | Partially adopted as an importable signed-differential state primitive | `hatSql.IncrementalMultiset` preserves exact per-key multiplicity, validates row identity and underflow atomically, and emits deterministic net deltas. Automatic planner wiring for every SQL operator remains caller-owned. See [MZ035_INCREMENTAL_MULTISET.md](MZ035_INCREMENTAL_MULTISET.md) and [BENCHMARK.md](BENCHMARK.md#mz-035-incremental-multiset). |
| --- | --- | --- | --- |
| ClickHouse | Join-derived runtime filter propagation | Partially adopted for the bounded spill hash join | Opt-in `SpillBloom` filters the larger right-side build stream with left-partition Bloom filters before chunk hash construction, preserves exact-map correctness, and exposes `RUNTIME JOIN FILTER` probe/skip metrics. General planner-wide filter exchange remains open. See [CHU14_RUNTIME_JOIN_FILTER.md](CHU14_RUNTIME_JOIN_FILTER.md) and [BENCHMARK.md#chu14-spill-runtime-join-filter](BENCHMARK.md#chu14-spill-runtime-join-filter). |
...
| Materialize | Incremental Top-K maintenance | Partially adopted as an importable exact weighted differential operator | `hatSql.IncrementalTopK` maintains keyed rows with signed `int64` multiplicities, SQL ordering, deterministic key ties, atomic batches, transition-only output, and bounded reusable selection scratch for lower-allocation updates. It requires a stable `DifferentialRow.Key`; automatic SQL plan integration and distributed exchange remain deferred. See [MZ037_INCREMENTAL_TOP_K.md](MZ037_INCREMENTAL_TOP_K.md), [MZ037_TOPK_SCRATCH_REUSE.md](MZ037_TOPK_SCRATCH_REUSE.md), and [BENCHMARK.md#mz-037-incremental-top-k-selection-scratch](BENCHMARK.md#mz-037-incremental-top-k-selection-scratch). |
...
| Materialize | Incremental percentile | Partially adopted as an importable exact weighted order-statistics operator | `hatSql.IncrementalPercentile` maintains keyed multiplicities in a SQL-ordered treap, applies signed batches atomically, answers nearest-rank percentile reads without a full relation sort, and uses a dedicated allocation-saving path for one-record updates. Automatic SQL plan integration, distributed arrangements, and approximate mergeable sketches remain deferred. See [MZ040_INCREMENTAL_PERCENTILE.md](MZ040_INCREMENTAL_PERCENTILE.md) and [BENCHMARK.md#mz-040-incremental-percentile](BENCHMARK.md#mz-040-incremental-percentile). |
...
| Materialize | Temporal validity filters | Partially adopted as a native SQL predicate and opt-in source index | `VALID_AT(at, valid_from, valid_to)` implements half-open `[valid_from, valid_to)` intervals with `NULL` open bounds and stays on the native scalar path for literal/field arguments. `HatTrie.CreateSQLJSONValidityIndex` adds lazy generation-aware interval candidates with admission and malformed-input fallback; frontier-aware pruning remains deferred. See [SQL_TEMPORAL_VALIDITY.md](SQL_TEMPORAL_VALIDITY.md) and [BENCHMARK.md#mz-009-temporal-validity-index](BENCHMARK.md#mz-009-temporal-validity-index). |
...
| Materialize | `TAIL`/`SUBSCRIBE` changefeed | Partially adopted as opt-in journal and SQL-result subscriptions | `CommandJournal.Subscribe` handles durable command tails; `QuerySubscriptions.SubscribeSQL` and `SubscribeDifferentialSQL` derive static `CACHE(...)` dependencies, reuse bounded snapshot/differential delivery, and can wrap payloads in bounded HMAC-SHA256 `SQLSubscriptionWireEnvelope` frames; `SQLSubscriptionWireKeyring` adds bounded key rotation; `SQLSubscriptionWireTransport` adds bounded length-prefixed context-aware delivery over a caller-owned `net.Conn`. Dialing, listener lifecycle, reconnect, and external authentication remain caller-owned. See [MZ010_JOURNAL_SUBSCRIPTIONS.md](MZ010_JOURNAL_SUBSCRIPTIONS.md), [MZ010_SQL_SUBSCRIPTIONS.md](MZ010_SQL_SUBSCRIPTIONS.md), [MZ010_CROSS_PROCESS_TRANSPORT.md](MZ010_CROSS_PROCESS_TRANSPORT.md), and [BENCHMARK.md#mz-010-cross-process-framed-transport](BENCHMARK.md#mz-010-cross-process-framed-transport). |
...
| Materialize | Snapshot blocking semantics | Adopted as an opt-in bounded readiness registry | `hatSql.SQLSnapshotReadinessRegistry` tracks monotone per-object progress, blocks a dependent until all required objects are ready, returns deterministic generation-stamped status snapshots, and wakes cancellable waits on progress, failure, or cancellation. Automatic connector wiring and durable readiness recovery remain caller-owned. See [M035_SNAPSHOT_BLOCKING.md](M035_SNAPSHOT_BLOCKING.md) and [BENCHMARK.md#m-u35-snapshot-blocking](BENCHMARK.md#m-u35-snapshot-blocking). |
| Materialize | Hydration progress and admission | Partially adopted as an opt-in arrangement readiness contract | Aggregate and join arrangements expose checkpoint/source progress through `HydrationStatus` and block explicit admission through cancellable `WaitForHydration`; existing bounded `Hydrate` workers and rebuild policy remain caller-owned, and status reads do not reread source rows. See [M036_ARRANGEMENT_HYDRATION_ADMISSION.md](M036_ARRANGEMENT_HYDRATION_ADMISSION.md) and [BENCHMARK.md#m-u36-arrangement-hydration-admission](BENCHMARK.md#m-u36-arrangement-hydration-admission). |
...
| Materialize | Incremental ordered arrangements | Partially adopted as an opt-in typed-table ordinal projection cache | `TypedTableColumnarCacheOptions.SortedOrderCache` admits bounded immutable `[]uint32` order vectors after repeated compatible single-field or composite `ORDER BY ... LIMIT` requests, including mixed direction patterns, and `TypedTableSortedArrangement.RowsRange` now binary-seeks validated scalar/composite bounds. It retains no duplicate row values and stays disabled by default; reusable cursors and arbitrary signed-update maintenance remain open. [C212_TYPED_TABLE_ORDER_CACHE.md](C212_TYPED_TABLE_ORDER_CACHE.md), [MZ038_SORTED_ARRANGEMENT_RANGE.md](MZ038_SORTED_ARRANGEMENT_RANGE.md), [BENCHMARK.md](BENCHMARK.md#mz-038-sorted-arrangement-range-reads) |
...
| ClickHouse | Read/write task profiler by table part and column | Adopted as an opt-in bounded collector | `SQLTaskProfiler` aggregates caller-supplied read/write rows, bytes, task counts, and duration by `(table, part, column, operation)` with bounded labels, deterministic snapshots, saturation, and least-recently-observed eviction. SQL execution defaults are unchanged. [C235_TASK_PROFILER.md](C235_TASK_PROFILER.md) |
...
| Materialize / ClickHouse | UDF purity and monotonicity classification | Adopted as explicit function metadata plus an opt-in literal-expression fast path | `FunctionDefinition` now carries conservative deterministic, pure, monotonicity, and retractable declarations; registry lookup returns defensive copies and persistence retains the metadata. `FunctionCapabilityResolver` folds pure deterministic literal UDF calls once per batch; no language inference or general expression folding is enabled. [MU032_UDF_CAPABILITIES.md](MU032_UDF_CAPABILITIES.md), [BENCHMARK.md](BENCHMARK.md#mu-032-udf-capability-classification), [BENCHMARK.md](BENCHMARK.md#mu-045-pure-literal-udf-fast-path) |
...
| Materialize | Historical subscription cancellation/checkpoints | Adopted as an opt-in acknowledged resume protocol | `QuerySubscription.Acknowledge` records consumer-applied state, `CloseWithCheckpoint` captures it atomically with cancellation, and `Resume`/`ResumeDifferential` avoid initial replay and suppress already-acknowledged frontiers. Existing `Close` and default subscriptions remain unchanged. [M034_HISTORICAL_SUBSCRIPTION_CHECKPOINTS.md](M034_HISTORICAL_SUBSCRIPTION_CHECKPOINTS.md), [BENCHMARK.md](BENCHMARK.md#m-u34-historical-subscription-checkpoints) |
...
| Tarantool | Full-text token-prefix lookup | Adopted as an opt-in SQL index path | `CONTAINS_PREFIX(field, prefix)` uses the existing JSON text postings plus a sorted token-key sidecar and binary-search prefix range. Candidates are unioned in source order and rechecked by the executor; empty or multi-token prefixes match no rows, while missing indexes retain the ordinary scan. Token-position phrase and ordered proximity search are available through the existing JSON text index and the new `MaterializedSource.BuildTextIndex` path; `TextIndexCatalog` adds opt-in context-aware `Put`/`Get` hooks for durable external storage of validated `HTI1` frames, while catalog durability and broader cross-field planning remain caller-owned. Missing indexes retain the ordinary scan. [SQL_TEXT_PREFIX_INDEX.md](SQL_TEXT_PREFIX_INDEX.md), [SQL_TEXT_PHRASE.md](SQL_TEXT_PHRASE.md), [TT024_POSITIONAL_TEXT_INDEX.md](TT024_POSITIONAL_TEXT_INDEX.md), [TT024_TEXT_INDEX_CATALOG.md](TT024_TEXT_INDEX_CATALOG.md) |
...
| ClickHouse | Grouping-set branch plan sharing and native one-pass aggregation | Partially adopted | `GROUPING SETS`, `ROLLUP`, and `CUBE` retain immutable source and relational descriptors while cloning only rewrite-local expressions. Simple aggregate projections automatically scan the filtered input once with shared compact state; richer query shapes fall back to branch-per-set execution. Multi-argument `GROUPING_ID` remains open. [CH041_GROUPING_PLAN_SHARING.md](CH041_GROUPING_PLAN_SHARING.md) |
...
| ClickHouse | Typed scalar JSON subcolumns | Adopted as an opt-in narrow columnar optimization, including M046 bounded Top-N and M047 grouped counts | `ColumnarJSONSubcolumn` stores typed scalar JSON paths with compact payloads plus presence/validity bitmaps. `ColumnarJSONSubcolumnSourceResolver` can serve them for literal `JSON_VALUE`, `JSON_EXISTS`, and `JSON_QUERY` paths; M046 uses an available scalar path for bounded `ORDER BY JSON_VALUE(...) LIMIT/OFFSET`, and M047 uses a compact first-seen group table for `GROUP BY JSON_VALUE(...) COUNT(*)`. Unsupported or unavailable cases preserve the existing map/row path. The measured 4,096-row filter is 3.92x faster, the bounded Top-N fixture is 70.4x faster, and the 10,000-row grouped-count fixture is 24.8x faster with 274x lower measured bytes and 392x fewer allocations. See [CH031_TYPED_JSON_SUBCOLUMNS.md](CH031_TYPED_JSON_SUBCOLUMNS.md), [M046_JSON_SUBCOLUMN_TOPN.md](M046_JSON_SUBCOLUMN_TOPN.md), [M047_TYPED_JSON_GROUP.md](M047_TYPED_JSON_GROUP.md), and [BENCHMARK.md](BENCHMARK.md#ch-031-typed-json-subcolumns). |
