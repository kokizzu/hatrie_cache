...
| ClickHouse | Asynchronous insert admission and completion status | Adopted as an opt-in journal API | `CommandJournal.SubmitAsyncCommand` admits journaled writes into the existing bounded group-commit worker; `Status`, `Done`, and `Wait` expose the durable-and-applied boundary. Queue-full backpressure, request ownership copies, replay, rollback, and idempotency remain explicit. Existing synchronous execution and defaults are unchanged. [ASYNC_COMMAND_SUBMISSION.md](ASYNC_COMMAND_SUBMISSION.md) |
