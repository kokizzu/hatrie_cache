...
| ClickHouse | Adaptive integer delta encoding | Adopted as an opt-in `hatDataStructure.EncodeUint64` codec | Monotone `uint64` values use first-value-plus-varint deltas when smaller than raw fixed-width bytes; unordered or high-entropy values fall back to raw encoding. Decoding is slower, so existing persistence and replication defaults remain unchanged. See [C247_UINT64_DELTA_CODEC.md](C247_UINT64_DELTA_CODEC.md). |
