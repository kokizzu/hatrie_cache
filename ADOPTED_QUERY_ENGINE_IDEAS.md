...
| ClickHouse | Typed scalar and complex JSON subcolumns | Adopted as an opt-in narrow columnar optimization | `ColumnarJSONSubcolumn` stores typed scalar paths with compact payloads and object/array paths in one packed JSON buffer plus row offsets, with presence/validity bitmaps. `ColumnarJSONSubcolumnSourceResolver` can serve literal `JSON_VALUE`, `JSON_EXISTS`, and `JSON_QUERY` paths; `JSON_VALUE` still rejects complex results and unsupported/unavailable cases preserve the existing map/row path. The existing 4,096-row scalar query remains 3.92x faster, while the matched 1,024-row complex `JSON_QUERY` query is 1.35x faster than its same-run control with 1.66x lower heap and 1.17x fewer allocations; one-time complex materialization costs 3.16 ms and 1.72 MB in the fixture. See [CH031_TYPED_JSON_SUBCOLUMNS.md](CH031_TYPED_JSON_SUBCOLUMNS.md) and [BENCHMARK.md](BENCHMARK.md#ch-u20-packed-complex-json-subcolumns). |
