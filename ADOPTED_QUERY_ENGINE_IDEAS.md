...
| Materialize | Incremental ordered arrangements | Partially adopted as an opt-in typed-table ordinal projection cache | `TypedTableColumnarCacheOptions.SortedOrderCache` admits bounded immutable `[]uint32` order vectors after repeated compatible single-field or composite `ORDER BY ... LIMIT` requests, including mixed direction patterns, reuses the existing columnar result path, and invalidates derived vectors on writes. It retains no duplicate row values and stays disabled by default; range cursors and arbitrary signed-update maintenance remain open. [C212_TYPED_TABLE_ORDER_CACHE.md](C212_TYPED_TABLE_ORDER_CACHE.md), [BENCHMARK.md](BENCHMARK.md#mz-038-typed-table-sorted-ordinal-projection) |
