...
| M-U09 | Durable frontier snapshots | `hatPipeline.FrontierRegistry` now exposes named monotone lower/upper frontiers, snapshots, waits, and bounded registration, but state is in-memory and not restored after restart. | Crash/restart recovery, version compatibility, atomic persistence, and no false completion. |
...
| M-U33 | As-of retention policy | Consumers can request historical frontiers, but retention is not coordinated with every subscription’s oldest needed timestamp. | No premature compaction, bounded storage, and removal. |
...
| T-U07 | Replica lag/RPO status object | Replication exposes queue/wire metrics, not one node-level applied sequence, lag, and RPO status object for every replica. | Bounded metrics, clock safety, and failover use. |
| T-U08 | Snapshot plus exact WAL coordinate | Backup APIs do not expose one online snapshot manifest paired with the exact journal coordinate required for bootstrap. | Concurrent writes, checksums, and restore rehearsal. |
...
| T-U11 | Per-space conflict policy | Conflict resolution exists, but each named collection cannot declare last-write, source-priority, or reject behavior. | Deterministic clocks, auditability, and migration. |
...
| T-U15 | Named space catalog | Typed schemas exist, but there is no Tarantool-like named space catalog combining tuple format, indexes, constraints, and version. | Field numbering, schema validation, and evolution. |
...
| T-U28 | Connection/schema lifecycle triggers | Peer APIs lack first-class connect, disconnect, shutdown, and schema-reload callbacks. | Callback ordering, concurrency, and cleanup. |
| T-U29 | Stream transaction API | Transactions exist, but there is no remote stream object with begin/commit/rollback and bounded multiplexing. | Cancellation, connection loss, and atomicity. |
...
| T-U40 | Tuple format version negotiation | Tuple formats validate fields, but clients cannot negotiate compatible format versions during rolling upgrades. | Unknown-field behavior and downgrade safety. |
| T-U41 | Snapshot-consistent iterator cursors | Ordered iterators have invalidation semantics, but no generic named-space cursor provides a stable snapshot across concurrent mutations. | Memory bound, invalidation, and repeatable ordering. |
| T-U42 | Cursor `after` pagination contract | Ordered access lacks a common serialized continuation token for efficient cross-request pagination. | Schema/version binding, tamper resistance, and no skipped rows. |
| T-U43 | Per-space memory quotas | Runtime memory reports exist, but allocations cannot be budgeted and rejected per named space/data structure. | Accurate attribution, no deadlock, and default zero overhead. |
...
| T-U45 | Per-space operation statistics | Cache-wide/key-level stats exist, but no named-space/index counters cover operations, bytes, hits/misses, and latency. | Label cardinality and allocation-free default path. |
| T-U46 | Index cardinality/hot-key statistics | Index APIs do not expose bounded cardinality, posting-length, and hot-key diagnostics for planning. | Privacy, sampling, and update overhead. |
| T-U47 | Cancellation/deadline propagation to peer calls | Contexts exist, but remote peer operations do not share a single cancellation, timeout, and cleanup contract across protocols. | Connection reuse, partial response, and leak tests. |
| T-U48 | Idempotent remote-call retry policy | Peer calls lack a method-aware retry/backoff policy tied to idempotency keys and fencing tokens. | No duplicate mutation, jitter, and observability. |
| T-U49 | Replica-set request hedging | Read routing selects candidates, but there is no bounded hedged-read policy that cancels slower replicas. | Tail-latency versus duplicate load and consistency. |
