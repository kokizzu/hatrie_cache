...
| CH-U03 | Versioned partial aggregate wire states | HyperLogLog and Count-Min Sketch states can be merged locally, but there is no versioned portable envelope for worker/node transfer. | Exact merge, schema/version rejection, bounded state size, and wire-size benchmark. |
