...
| CH-046 | Native wire protocol framing | Partially adopted: opt-in `application/x-hatrie-columnar` typed block streaming now carries explicit per-column frames and cumulative progress, supports bounded projection decoding, and preserves NDJSON/RowBinary defaults. Compression and dictionary wire encodings remain open. See [CH046_NATIVE_WIRE_PROTOCOL.md](CH046_NATIVE_WIRE_PROTOCOL.md). | High |
