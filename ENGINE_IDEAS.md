...
| CH-048 | SIMD generic predicates | No vector kernels for common comparisons outside the existing narrow vector path. | Medium |
...
| TT-016 | Disk-space reserve admission | Writes do not fail early based on a configured compaction/restore reserve. | Low |
