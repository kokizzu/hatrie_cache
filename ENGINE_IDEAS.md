...
| MZ-026 | Dictionary arrangement compression | Partially adopted as opt-in `DictionaryEncodeGroups` for typed aggregates and `DictionaryEncoded` sorted arrangements; `DictionaryAdaptive` now admits bounded low-cardinality sorted strings and demotes safely on growth, with exactness and benchmark coverage. Automatic selection for other paths remains open. | Medium |
...
| MZ-028 | Adaptive arrangement compaction | Partially adopted: legacy typed aggregates now share cached ordered group references with dictionary aggregates; one new group after a read is inserted into the existing order, while batched additions and deletions retain the full-sort fallback. A full memory/update-rate feedback policy remains open. See [MZ028_ADAPTIVE_ARRANGEMENT.md](MZ028_ADAPTIVE_ARRANGEMENT.md). | Medium |
...
| MZ-038 | Incremental order arrangement | Partially adopted: opt-in typed-table columnar caches can admit bounded single-field or composite `[]uint32` order projections, including mixed directions, after repeated `ORDER BY ... LIMIT` requests; range cursors and fully incremental arbitrary updates remain. | High |
...
| TT-021 | RTREE spatial index | Partially adopted: `hatDataStructure.PackedRTree[T]` remains the compact immutable API, and opt-in `MutablePackedRTree[T]` adds a packed base plus mutable delta with explicit compaction; SQL planner wiring and persistence remain open. See [TT021_MUTABLE_PACKED_RTREE.md](TT021_MUTABLE_PACKED_RTREE.md). | High |
