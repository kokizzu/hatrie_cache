...
| CH-046 | Native wire protocol framing | Partially adopted: opt-in `application/x-hatrie-columnar` typed block streaming now carries explicit per-column frames and cumulative progress, supports bounded projection decoding, and preserves NDJSON/RowBinary defaults. Versioned per-column Flate compression with bounded decoding and an explicit raw v1 fallback is available, but benchmarks keep raw v1 as the default because compression is materially more CPU/allocation-heavy; dictionary encoding remains open. See [CH046_NATIVE_WIRE_PROTOCOL.md](CH046_NATIVE_WIRE_PROTOCOL.md) and [CH046_WIRE_COMPRESSION.md](CH046_WIRE_COMPRESSION.md). | High |
