...
| CH-041 | `GROUPING SETS`/`ROLLUP`/`CUBE` | Partially adopted: existing multi-level expansion now supports `GROUPING(expr)` identifiers and multi-argument `GROUPING_ID` folded per branch; native one-pass grouping remains open. See [CH041_GROUPING_ID.md](CH041_GROUPING_ID.md). | High |
...
| CH-046 | Native wire protocol framing | Partially adopted: opt-in `application/x-hatrie-columnar` typed block streaming now carries explicit per-column frames and cumulative progress, supports bounded projection decoding, and preserves NDJSON/RowBinary defaults. Versioned Flate and bounded low-cardinality string dictionary encodings are available with raw v1 fallback; raw remains the default because the bandwidth wins carry measurable CPU/allocation cost. See [CH046_NATIVE_WIRE_PROTOCOL.md](CH046_NATIVE_WIRE_PROTOCOL.md) and [CH046_WIRE_COMPRESSION.md](CH046_WIRE_COMPRESSION.md). | High |
...
| MZ-009 | Temporal validity filters | Partially adopted: `VALID_AT(at, valid_from, valid_to)` provides first-class half-open validity semantics, native scalar execution, and safe literal lower-bound candidate pushdown through existing range indexes; index maintenance and frontier-aware pruning remain open. See [MZ009_VALIDITY_INDEX.md](MZ009_VALIDITY_INDEX.md). | High |
...
| MZ-028 | Adaptive arrangement compaction | Partially adopted: legacy typed aggregates now share cached ordered group references with dictionary aggregates, rebuilding only after group membership changes and avoiding repeated full sorts on stable reads; a full memory/update-rate feedback policy remains open. See [MZ028_ADAPTIVE_ARRANGEMENT.md](MZ028_ADAPTIVE_ARRANGEMENT.md). | Medium |
...
| MZ-045 | Workload plan equivalence | Partially adopted: bounded `SQLCompiledQueryCache` canonicalizes equivalent token streams, and compiled immutable query trees now prepare normalized arrangement workloads once for repeated `EXPLAIN`/scan/join recommendation reads. Literal/schema namespaces and parameter-bound clone invalidation remain protected; automatic planner-wide arrangement reuse remains open. See [MZ045_WORKLOAD_PLAN_EQUIVALENCE.md](MZ045_WORKLOAD_PLAN_EQUIVALENCE.md). | Low |
