...
normalizes hyphens to underscores. A monotonicity or retraction declaration
without `Deterministic: true` is rejected. Unknown values are rejected rather
than silently downgraded. Existing definitions with none of these fields keep
the conservative unknown state.
...
- No automatic SQL planner fast path is enabled yet. Existing query execution
  and runtime sandbox behavior are unchanged.
...
allocation to the hot evaluation path. The lookup cost is the intentional
price of returning independent slices; callers should cache definitions when
planning repeated executions.
