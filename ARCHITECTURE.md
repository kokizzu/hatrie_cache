...
| `hatrie_cache/hat/hatDataStructure` | Standalone compact algorithms: Fenwick tree, Quantile Sketch, Roaring Bitmap, Sparse Bitset, HyperLogLog, and Bloom Filter shape calculation | Root keeps cache storage, snapshots, replication, and command adapters |
...
| Data structures | `*_filter.go`, `*_sketch.go`, `fenwick_tree.go`, `radix_tree.go`, `sparse_bitset.go`, `roaring_bitmap.go` | Each has command, snapshot, binary replication, storage, and compact-memory integrations. Fenwick, Quantile Sketch, Roaring Bitmap, Sparse Bitset, and HyperLogLog reusable algorithms are in `hat/hatDataStructure`; their cache adapters remain here. |
