...
static size_t format_key(char* out, size_t out_size, size_t index)
{
...
int main(int argc, char** argv)
{
    ...
    for (index = 0; index < keys; ++index) {
        size_t length = format_key(key, sizeof(key), index);
    ...
    for (index = 0; index < LOOKUP_KEY_COUNT; ++index) {
        ...
        lookup_lengths[index] = format_key(lookup_keys[index], sizeof(lookup_keys[index]), key_index);
    ...
    printf("keys=%zu lookup_operations=%zu tryget_seconds=%.9f get_seconds=%.9f size=%zu max_rss_kib=%ld\n",
           keys,
           lookup_operations,
           tryget_seconds,
           get_seconds,
