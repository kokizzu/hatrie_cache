...
| Case-sensitive UTF-8 binary text collation is stable for comparison and ordering; explicit `NULLS FIRST`/`NULLS LAST` overrides null placement without changing defaults in both output and window ordering | [`TestExecuteSQLQueryUsesCaseSensitiveUTF8BinaryStringCollation`](sql_production_test.go), [`TestExecuteSQLQuerySupportsExplicitNullOrder`](sql_production_test.go), [`TestExecuteSQLQueryWindowOrderHonorsExplicitNullOrder`](sql_production_test.go) |
...
| Searched and simple `CASE`, null-sensitive matching, lazy selected-branch evaluation, keyword inventory, and clear missing-`THEN`/`END` diagnostics | [`TestExecuteSQLQuerySupportsCaseExpressions`](sql_production_test.go), [`TestSQLAcceptedKeywordInventory`](sql_function_test.go), [`TestSQLKeywordInventoryReportsContextualDiagnostics`](sql_function_test.go) |
| `IN` and `NOT IN` membership, including a `NULL` list member producing unknown rather than a false match | [`TestExecuteSQLQuerySupportsInWithNullSemantics`](sql_production_test.go) |
| Inclusive `BETWEEN` / `NOT BETWEEN` predicates, including null-bound unknown semantics | [`TestExecuteSQLQuerySupportsBetweenWithNullSemantics`](sql_production_test.go) |
...
| True row-by-row execution for stream-compatible `CACHE`/`VALUES` queries, including one held-and-released source snapshot, bounded top-N, unbounded external-sort merge, and first-occurrence external-`DISTINCT` streaming, NDJSON HTTP records, streamability diagnostics, and the one-response generic Go `QueryRows` SDK | [`TestExecuteSQLQueryRowsStreamsCompatibleSourceRows`](sql_production_test.go), [`TestExecuteSQLQueryRowsHoldsAndReleasesSnapshot`](sql_production_test.go), [`TestExecuteSQLQueryRowsStreamsBoundedTopNOrdering`](sql_production_test.go), [`TestExecuteSQLQueryRowsStreamsExternalSort`](sql_production_test.go), [`TestExecuteSQLQueryRowsStreamsExternalSortValues`](sql_production_test.go), [`TestExecuteSQLQueryRowsStreamsExternalSortRandomized`](sql_production_test.go), [`TestExecuteSQLQueryRowsExternalSortCleansUpOnFailure`](sql_production_test.go), [`TestExecuteSQLQueryRowsStreamsExternalDistinct`](sql_production_test.go), [`TestExecuteSQLQueryRowsStreamsExternalDistinctValues`](sql_production_test.go), [`TestExecuteSQLQueryRowsExternalDistinctMergesOrdinalRuns`](sql_production_test.go), [`TestExecuteSQLQueryRowsStreamsExternalDistinctRandomized`](sql_production_test.go), [`TestExecuteSQLQueryRowsExternalDistinctCleansUpOnFailure`](sql_production_test.go), [`TestMonitoringSQLRouteStreamsNDJSONRows`](sql_http_test.go), [`TestSQLConnQueryRowsDecodesAndStopsOnCallbackError`](sql_client_test.go), [`TestSQLConnQueryRowsConsumesOneNDJSONResponse`](sql_client_test.go) |
| Direct bounded external `GROUP BY` reads `CACHE` input incrementally and streams its final group merge to `QueryRows`; it also falls back when a compatible ordered-index interface is present but the requested index is absent, and cleans temporary runs on callback/resource-limit failure | [`TestExecuteSQLQueryRowsStreamsExternalGroupAggregate`](sql_production_test.go), [`TestExecuteSQLQueryRowsExternalGroupFallsBackWhenOrderedIndexMissing`](sql_production_test.go), [`TestExecuteSQLQueryRowsExternalGroupCleansUpOnFailure`](sql_production_test.go), [`TestExecuteSQLQueryContextStreamsSpilledDirectGroupedAggregates`](sql_production_test.go) |
| Streamed queries produce one observer event with counted emitted bytes/rows and output columns, including callback failure without retaining result rows | [`TestExecuteSQLQueryRowsObserverReportsStreamedOutput`](sql_production_test.go), [`TestExecuteSQLQueryObserverIncludesSafeOperatorCounters`](sql_production_test.go) |
| Streamability rejections explain the matching ordered-index, bounded top-N, or external spill setting rather than only naming a global operator | [`TestExecuteSQLQueryRowsExplainsUnavailableGlobalStreaming`](sql_production_test.go) |
| One direct scalar `UNION`, `INTERSECT`, or `EXCEPT` streams both input sources through bounded external set/ordinal merge runs without materializing a source or result slice | [`TestExecuteSQLQueryRowsStreamsExternalSetOperations`](sql_production_test.go), [`TestExecuteSQLQueryRowsExternalSetMergesOrdinalRuns`](sql_production_test.go) |
