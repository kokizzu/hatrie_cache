...
| Optional JSON equality index, `INDEX SCAN` planning, refresh after a cache write, and forwarding through authenticated `/api/sql` | [`TestHatTrieOptionalSQLJSONFieldIndexRefreshesAndPlansIndexScan`](sql_production_test.go), [`TestMonitoringSQLRouteUsesHatTrieJSONFieldIndex`](sql_http_test.go) |
