...
| Bounded prepared-query templates cache parsed parameterized SQL, bind fresh values for every execution, and preserve `CACHE`, `VALUES`, and nested-query parameters without cross-request state | [`TestSQLPreparedQueryCacheReusesImmutableTemplateAndBindsFreshParameters`](sql_production_test.go), [`TestSQLPreparedQueryCacheBindsValuesAndNestedQueryParameters`](sql_production_test.go) |
...
| True row-by-row execution for stream-compatible `CACHE`/`VALUES` queries, NDJSON HTTP records, streamability diagnostics, and the one-response generic Go `QueryRows` SDK | [`TestExecuteSQLQueryRowsStreamsCompatibleSourceRows`](sql_production_test.go), [`TestMonitoringSQLRouteStreamsNDJSONRows`](sql_http_test.go), [`TestSQLConnQueryRowsDecodesAndStopsOnCallbackError`](sql_client_test.go), [`TestSQLConnQueryRowsConsumesOneNDJSONResponse`](sql_client_test.go) |
| Streamed indexed equality INNER/LEFT CACHE joins probe the right index per left row without materializing a source or result slice | [`TestExecuteSQLQueryRowsStreamsIndexedInnerAndLeftJoin`](sql_production_test.go) |
...
| Optional JSON equality/range indexes; longest matching multi-field equality index and refreshed cardinality statistics; selective `AND` predicates; `INDEX SCAN`/`INDEX JOIN` planning (inner and null-preserving left joins); refresh after a cache write; and forwarding through authenticated `/api/sql` | [`TestHatTrieOptionalSQLJSONFieldIndexRefreshesAndPlansIndexScan`](sql_production_test.go), [`TestHatTrieOptionalSQLJSONFieldIndexSupportsRangePredicates`](sql_production_test.go), [`TestHatTrieOptionalSQLJSONFieldIndexSelectsConjunctivePredicate`](sql_production_test.go), [`TestHatTrieSQLCompositeJSONIndexPlansAndReportsStatistics`](sql_production_test.go), [`TestHatTrieOptionalSQLJSONFieldIndexProbesInnerJoin`](sql_production_test.go), [`TestHatTrieOptionalSQLJSONFieldIndexProbesLeftJoin`](sql_production_test.go), [`TestMonitoringSQLRouteUsesHatTrieJSONFieldIndex`](sql_http_test.go), [`TestMonitoringSQLRouteUsesHatTrieCompositeJSONIndex`](sql_http_test.go) |
...
| Deterministic generated-reference cases plus SQLite differential cases for INNER/LEFT joins, grouping, windows, timestamps, and recursive CTEs | [`TestSQLGeneratedReferenceCasesForJoinsGroupsAndSets`](sql_production_test.go), [`TestSQLDifferentialAgainstSQLiteForJoinsGroupsAndWindows`](sql_production_test.go) |
