...
backup_abs=$(absolute_existing_dir "$backup_dir")
data_abs=$(absolute_target_dir "$data_dir")

mkdir -p "$data_abs"
if has_entries "$data_abs" && ! bool_true "$overwrite"; then
	fail "DATA_DIR is not empty: $data_dir; set RESTORE_OVERWRITE=true to copy into it"
fi

cp -a "$backup_abs/." "$data_abs/"
echo "restore: copied $backup_abs to $data_abs"
