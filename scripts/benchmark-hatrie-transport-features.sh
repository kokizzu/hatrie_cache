...
bench=${HATRIE_TRANSPORT_BENCH:-^BenchmarkCommandTransportFeature$}
benchtime=${BENCHTIME:-}
count=${COUNT:-1}
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/hatrie-transport-bench.XXXXXX")
...
printf 'HAT-trie transport benchmark: bench=%s benchtime=%s count=%s\n\n' "$bench" "${benchtime:-default}" "$count"
