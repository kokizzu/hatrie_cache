...
repo_dir=$PWD
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/hatrie-chu08-benchmark.XXXXXX")
trap 'rm -rf "$tmp_dir"' EXIT

git archive --format=tar HEAD | tar -xf - -C "$tmp_dir"
cp "$repo_dir/hat/hatCache/ch008_auto_result_cache_benchmark_test.go" "$tmp_dir/hat/hatCache/"
...
cd "$tmp_dir"
go test ./hat/hatCache -run '^$' -bench '^BenchmarkCHU08' -benchmem -count=5
