...
tmp_dir=$(mktemp -d /tmp/hatrie-cache-race-chu09.XXXXXX)
...
(cd "$tmp_dir" && go test -race ./hat/hatSql ./hat/hatCache -run 'TestSQLResultCachePersistence|TestHatTrieSQLResultCachePersistence' -count=1)
