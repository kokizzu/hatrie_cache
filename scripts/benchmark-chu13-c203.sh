...
repo_root=$(pwd)
tmp_root=$(mktemp -d /tmp/hatrie-cache-chu13-benchmark.XXXXXX)
cleanup() {
...
cd "$tmp_root"
go test ./hat/hatDataStructure -run '^TestCHU13PhrasePostingsMemoryReport$' -count=1 -v
go test ./hat/hatDataStructure -run '^$' -bench '^BenchmarkCHU13' -benchmem -benchtime=100ms -count=5
