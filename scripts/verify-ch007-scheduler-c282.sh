...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/hatrie-cache-ch007-scheduler-verify-XXXXXX")
...
cd "$tmp_dir"
go test ./hat/hatSql -run 'TestCH007(TTL|ProcessingTTL)' -count=1 -timeout 15s
go test -race ./hat/hatSql -run 'TestCH007(TTL|ProcessingTTL)' -count=1 -timeout 30s
go vet ./hat/hatSql
