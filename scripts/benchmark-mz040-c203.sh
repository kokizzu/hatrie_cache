...
repo_root=$(pwd)
tmp_root=$(mktemp -d /tmp/hatrie-cache-mz040-benchmark.XXXXXX)
cleanup() {
...
(cd "$tmp_root" && go test ./hat/hatSql -run '^$' -bench '^(BenchmarkRecursiveReachabilityMaintenance|BenchmarkMZ040RecursiveReachabilityApply)$' -benchmem -benchtime=20ms -count=3)
